...
def open_file_test_util13(file_name, mode='r', encoding=None, **kwargs):
...
class test_util13(wttest.WiredTigerTestCase, suite_subprocess):
    ...
    def compare_config(self, expected_cfg, actual_cfg):
        ...
        match = all(item in list(da.items()) for item in list(dx.items()))
        if match == False:
            print("MISMATCH:")
            print("Original dict: ")
            print(da)
    ...
    def compare_files(self, expect_subset, dump_out):
        ...
        for l1, l2 in zip(open_file_test_util13(expect_subset, "rb"), open_file_test_util13(dump_out, "rb")):
            if isconfig:
    ...
    def test_dump_config(self):
        ...
        with open_file_test_util13(expectfile, "w") as expectout:
