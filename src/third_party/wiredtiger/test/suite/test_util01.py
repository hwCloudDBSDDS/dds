...
def open_file_test_util01(file_name, mode='r', encoding=None, **kwargs):
...
class test_util01(wttest.WiredTigerTestCase, suite_subprocess):
    ...
    def compare_files(self, filename1, filename2):
        ...
        for l1, l2 in zip(open_file_test_util01(filename1, "rb"), open_file_test_util01(filename2, "rb")):
            if isconfig:
    ...
    def dump(self, usingapi, hexoutput):
        ...
        with open_file_test_util01("expect.out", "w") as expectout:
            if not usingapi:
        ...
        with open_file_test_util01("dump.out", "w") as dumpout:
            if usingapi:
                if hexoutput:
                    dumpopt = "dump=hex"
                else:
