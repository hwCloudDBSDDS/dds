...
struct Word {
    ...
    Word(const string& w, uint32_t o) : word(w), offset(o) {}
...
// [left, right]
inline Word GetWordFromRunes(const string& s,
                             RuneStrArray::const_iterator left,
                             RuneStrArray::const_iterator right) {
...
inline string GetStringFromRunes(const string& s,
                                 RuneStrArray::const_iterator left,
                                 RuneStrArray::const_iterator right) {
    assert(right->offset >= left->offset);
    uint32_t len = right->offset - left->offset + right->len;
...
inline void GetWordsFromWordRanges(const string& s,
                                   const vector<WordRange>& wrs,
                                   vector<Word>& words) {
