    ...
    public:
        ...
        void rank(WordMap& ws, size_t rankTime = 10) {
            ...
            for (Graph::iterator edges = graph.begin(); edges != graph.end(); ++edges) {
                ...
                for (Edges::iterator edge = edges->second.begin(); edge != edges->second.end();
                     ++edge) {
            ...
            for (size_t i = 0; i < rankTime; i++) {
                for (NodeSet::iterator node = nodeSet.begin(); node != nodeSet.end(); node++) {
                    double s = 0;
                    for (Edges::iterator edge = graph[*node].begin(); edge != graph[*node].end();
                         edge++)
...
public:
    ...
    TextRankExtractor(const DictTrie* dictTrie, const HMMModel* model, const string& stopWordPath)
        : segment_(dictTrie, model) {
        LoadStopWordDict(stopWordPath);
    }
    TextRankExtractor(const Jieba& jieba, const string& stopWordPath)
    ...
    void Extract(const string& sentence,
