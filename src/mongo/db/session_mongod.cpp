...
void updateSessionEntry(OperationContext* opCtx, const UpdateRequest& updateRequest) {
    ...
    auto matcher =
        fassert(90427, MatchExpressionParser::parse(updateRequest.getQuery(), std::move(expCtx)));
...
SessionMongoD::SessionMongoD(LogicalSessionId sessionId) {
    _sessionId = std::move(sessionId);
}

void SessionMongoD::setCurrentOperation(OperationContext* currentOperation) {
    stdx::lock_guard<stdx::mutex> lk(_mutex);
    invariant(!_currentOperation);
    _currentOperation = currentOperation;
}

void SessionMongoD::clearCurrentOperation() {
    stdx::lock_guard<stdx::mutex> lk(_mutex);
    invariant(_currentOperation);
    _currentOperation = nullptr;
}

void SessionMongoD::refreshFromStorageIfNeeded(OperationContext* opCtx) {
...
void SessionMongoD::beginOrContinueTxn(OperationContext* opCtx,
                                       TxnNumber txnNumber,
...
void SessionMongoD::beginOrContinueTxnOnMigration(OperationContext* opCtx, TxnNumber txnNumber) {
    invariant(!opCtx->getClient()->isInDirectClient());
    invariant(!opCtx->lockState()->isLocked());

    stdx::lock_guard<stdx::mutex> lg(_mutex);
    _beginOrContinueTxnOnMigration(opCtx, lg, txnNumber);
}

void SessionMongoD::setSpeculativeTransactionOpTime(OperationContext* opCtx,
...
void SessionMongoD::onWriteOpCompletedOnPrimary(OperationContext* opCtx,
                                                TxnNumber txnNumber,
...
bool SessionMongoD::onMigrateBeginOnPrimary(OperationContext* opCtx,
                                            TxnNumber txnNumber,
                                            StmtId stmtId) {
...
void SessionMongoD::onMigrateCompletedOnPrimary(OperationContext* opCtx,
                                                TxnNumber txnNumber,
                                                std::vector<StmtId> stmtIdsWritten,
                                                const repl::OpTime& lastStmtIdWriteOpTime,
...
repl::OpTime SessionMongoD::getLastWriteOpTime(TxnNumber txnNumber) const {
    stdx::lock_guard<stdx::mutex> lg(_mutex);
    _checkValid(lg);
...
boost::optional<repl::OplogEntry> SessionMongoD::checkStatementExecuted(OperationContext* opCtx,
                                                                        TxnNumber txnNumber,
                                                                        StmtId stmtId) const {
...
bool SessionMongoD::checkStatementExecutedNoOplogEntryFetch(TxnNumber txnNumber,
                                                            StmtId stmtId) const {
    stdx::lock_guard<stdx::mutex> lg(_mutex);
    return bool(_checkStatementExecuted(lg, txnNumber, stmtId));
}

void SessionMongoD::_beginOrContinueTxn(OperationContext* opCtx,
                                        WithLock wl,
                                        TxnNumber txnNumber,
                                        boost::optional<bool> autocommit,
                                        boost::optional<bool> startTransaction) {
...
void SessionMongoD::_checkTxnValid(WithLock, TxnNumber txnNumber) const {
...
SessionMongoD::TxnResources::TxnResources(OperationContext* opCtx, bool keepTicket) {
...
SessionMongoD::TxnResources::~TxnResources() {
...
void SessionMongoD::TxnResources::release(OperationContext* opCtx) {
...
void SessionMongoD::stashTransactionResources(OperationContext* opCtx) {
    if (opCtx->getClient()->isInDirectClient()) {
        return;
    }
...
void SessionMongoD::unstashTransactionResources(OperationContext* opCtx,
...
void SessionMongoD::abortArbitraryTransaction(OperationContext* opCtx) {
...
void SessionMongoD::abortArbitraryTransactionIfExpired(OperationContext* opCtx) {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
...
void SessionMongoD::abortActiveTransaction(OperationContext* opCtx) {
    stdx::lock_guard<stdx::mutex> lock(_mutex);
...
void SessionMongoD::_abortTransaction(OperationContext* opCtx, WithLock wl) {
...
void SessionMongoD::_beginOrContinueTxnOnMigration(OperationContext* opCtx,
                                                   WithLock wl,
                                                   TxnNumber txnNumber) {
    _checkValid(wl);
    _checkTxnValid(wl, txnNumber);
...
void SessionMongoD::_setActiveTxn(OperationContext* opCtx, WithLock wl, TxnNumber txnNumber) {
...
void SessionMongoD::addTransactionOperation(OperationContext* opCtx,
...
std::vector<repl::ReplOperation> SessionMongoD::endTransactionAndRetrieveOperations(
...
void SessionMongoD::commitTransaction(OperationContext* opCtx) {
...
void SessionMongoD::_commitTransaction(stdx::unique_lock<stdx::mutex> lk, OperationContext* opCtx) {
    invariant(_txnState == MultiDocumentTransactionState::kInProgress);
...
BSONObj SessionMongoD::reportStashedState() const {
    BSONObjBuilder builder;
    reportStashedState(&builder);
    return builder.obj();
}

void SessionMongoD::reportStashedState(BSONObjBuilder* builder) const {
...
void SessionMongoD::reportUnstashedState(repl::ReadConcernArgs readConcernArgs,
...
void SessionMongoD::_reportTransactionStats(WithLock wl,
                                            BSONObjBuilder* builder,
                                            repl::ReadConcernArgs readConcernArgs) const {
    _singleTransactionStats.report(builder, readConcernArgs);
}

std::string SessionMongoD::_transactionInfoForLog(const SingleThreadedLockStats* lockStats,
...
void SessionMongoD::_logSlowTransaction(WithLock wl,
...
void SessionMongoD::_checkValid(WithLock) const {
    uassert(ErrorCodes::ConflictingOperationInProgress,
...
void SessionMongoD::_checkIsActiveTransaction(WithLock,
                                              TxnNumber txnNumber,
...
boost::optional<repl::OpTime> SessionMongoD::_checkStatementExecuted(WithLock wl,
                                                                     TxnNumber txnNumber,
                                                                     StmtId stmtId) const {
    _checkValid(wl);
...
Date_t SessionMongoD::_getLastWriteDate(WithLock wl, TxnNumber txnNumber) const {
    _checkValid(wl);
    _checkIsActiveTransaction(wl, txnNumber, false);
...
UpdateRequest SessionMongoD::_makeUpdateRequest(WithLock,
...
void SessionMongoD::_registerUpdateCacheOnCommit(OperationContext* opCtx,
                                                 TxnNumber newTxnNumber,
                                                 std::vector<StmtId> stmtIdsWritten,
                                                 const repl::OpTime& lastStmtIdWriteOpTime) {
    opCtx->recoveryUnit()->onCommit([
        ...
        if (newTxnNumber > _activeTxnNumber) {
            ...
            _beginOrContinueTxn(opCtx, lg, newTxnNumber, boost::none, boost::none);
        }
...
boost::optional<repl::OplogEntry> SessionMongoD::createMatchingTransactionTableUpdate(
