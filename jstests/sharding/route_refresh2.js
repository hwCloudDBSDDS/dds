(function() {
    var s1 = new ShardingTest({name: "route_refresh2", shards: 2, mongos: 2});
    var s2 = s1._mongos[1];
    ...
    if (s1primaryshard != s1.getPrimaryShard("test2").name) {
        assert.commandWorked(s1.getDB("test2").dropDatabase());
    }

    if (s1primaryshard != s1.getPrimaryShard("test3").name) {
        assert.commandWorked(s1.getDB("test3").dropDatabase());
    }

    if (s1primaryshard != s1.getPrimaryShard("test4").name) {
        assert.commandWorked(s1.getDB("test4").dropDatabase());
    }

    if (s1primaryshard != s1.getPrimaryShard("test5").name) {
        assert.commandWorked(s1.getDB("test5").dropDatabase());
    }

    if (s1primaryshard != s1.getPrimaryShard("test6").name) {
        assert.commandWorked(s1.getDB("test6").dropDatabase());
    }

    if (s1primaryshard != s1.getPrimaryShard("test7").name) {
        assert.commandWorked(s1.getDB("test7").dropDatabase());
    }
    ...
    // s2 update new database
    assert.writeOK(s2.getDB("test1").foo.update({}, {$set: {num: 5}}));

    // s1 check new database data is new or not
    assert.eq(1, s1.getDB("test1").foo.find({num: 5}).count(), "route flush failed");
    assert.eq(1, s2.getDB("test1").foo.find({num: 5}).count(), "route flush failed");
