...
function runAllRoleManagementCommandsTests(conn, writeConcern) {
    ...
    adminUserAdmin.createUser({
        ...
        roles: ['userAdminAnyDatabase'], "passwordDigestor": "server"
                              ...
                              writeConcern);
    adminUserAdmin.auth('admin', 'Password@a1b');
    testUserAdmin.getSiblingDB('admin').createUser(
        {user: 'monitor', pwd: 'Password@a1b', roles: [], "passwordDigestor": "server"},
        writeConcern);
    var db = conn.getDB('test');
    assert(db.getSiblingDB('admin').auth('monitor', 'Password@a1b'));
    ...
    (function testCreateRole() {
        ...
        testUserAdmin.getSiblingDB('admin').updateUser(
            'monitor', {roles: [{'role': 'testRole1', 'db': 'test'}]}, writeConcern);
        ...
        testUserAdmin.getSiblingDB('admin').updateUser(
            'monitor', {roles: [{'role': 'testRole2', 'db': 'test'}]}, writeConcern);
        assert.throws(function() {
            db.foo.findOne();
        });
        ...
        testUserAdmin.getSiblingDB('admin').updateUser(
            'monitor', {roles: [{'role': 'testRole3', 'db': 'test'}]}, writeConcern);
        assert.doesNotThrow(function() {
            db.foo.findOne();
        });
    ...
    (function testRolesInfo() {
        ...
        /*        res = testUserAdmin.runCommand({rolesInfo: "testRole1", showPrivileges:
           'asUserFragment'});
    ...
    (function testDropRole() {
        ...
        testUserAdmin.getSiblingDB('admin').grantRolesToUser(
            'monitor', [{'role': 'testRole4', 'db': 'test'}], writeConcern);
