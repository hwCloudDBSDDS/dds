...
(function() {
    ...
    var testUserSpecialCharacters = function() {
        ...
        assert(adminDB.auth({user: '~`!@#$%^&*()-_+={}[]||;:",.//><', pwd: 'Password@a1b'}));
        assert(adminDB.logout());
    };
    ...
    var testUserAndDatabaseAtSymbolConflation = function() {
        ...
        bcDB.createUser({
            user: 'a',
            pwd: 'Password@a1b2',
            roles: [{role: 'readWrite', db: 'b@c'}], "passwordDigestor": "server"
        });
        ...
        cDB.createUser({
            user: 'a@b',
            pwd: 'Password@a1b1',
            roles: [{role: 'readWrite', db: 'c'}], "passwordDigestor": "server"
        });

        assert(adminDB.logout());

        // Ensure they cannot authenticate to the wrong database.
        assert(!bcDB.auth('a@b', 'Password@a1b1'));
        assert(!bcDB.auth('a@b', 'Password@a1b2'));
        assert(!cDB.auth('a', 'Password@a1b1'));
        assert(!cDB.auth('a', 'Password@a1b2'));

        // Ensure that they can both successfully authenticate to their correct database.
        assert(cDB.auth('a@b', 'Password@a1b1'));
        assert.writeOK(cDB.col.insert({data: 1}));
        assert.writeError(bcDB.col.insert({data: 2}));
        assert(cDB.logout());

        assert(bcDB.auth('a', 'Password@a1b2'));
