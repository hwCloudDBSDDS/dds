(function() {
    'use strict';
        ...
        assert.commandWorked(admin.runCommand({shardCollection:str2+".foo",unique:false,key:{a:"hashed"}}));
        jsTest.log("-------------------insert data-------------------");
        
        printShardingStatus(st.config,false);
        jsTest.log("-------------------splite-------------------");
        var ransp = 500; //Math.floor(Math.random()*1000);
		...
		var res = print.captureAllOutput(function() {
            return printShardingStatus(st.config, false);
        });
        var output = res.output.join("\n");
        jsTestLog(output);
        assert(output.includes("NumberLong(0) } -->> { \"a\" : "+ransp));
		assert(output.includes("{ \"a\" : "+ransp));
