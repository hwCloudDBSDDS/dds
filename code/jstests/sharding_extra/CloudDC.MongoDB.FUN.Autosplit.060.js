(function() {
 ...
 for (var j=0;j<10000;j++){
     assert.writeOK(coll.update({b: (-1-j)},{$set:{b:(-10000-j)}}, false,true));
     //coll1.update({b: (-1-j)},{$set:{b:(-10000-j)}});		
 }
 ...
 for (var j=0;j<801;j++){
     ...
     sleep (5*100);
     jsTest.log("waiting...");
     assert.lt(j,800,"Timeout");
 }
 for (var j=0;j<10000;j++){
     assert.writeOK(coll1.update({b: (-1-j)},{$set:{b:(-10000-j)}}, false,true));
     //coll1.update({b: (-1-j)},{$set:{b:(-10000-j)}});		
 }
 //判断chunk增加
 sleep (20*1000);
 ...
 assert.eq(12000,coll.count());
 assert.eq(12000,coll1.count());
