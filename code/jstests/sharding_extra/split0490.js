(function() {
    'use strict';
        ...
        assert.eq(num,2);
		st.stopBalancer();
		assert.commandWorked(admin.runCommand({moveChunk: coll + "", find: {a: 1}, to: "shard0002"}));
		assert.commandWorked(admin.runCommand({moveChunk: coll + "", find: {a: 1}, to: "shard0001"}));
		assert.commandWorked(admin.runCommand({moveChunk: coll + "", find: {a: 100}, to: "shard0001"}));
		assert.commandWorked(admin.runCommand({moveChunk: coll + "", find: {a: 100}, to: "shard0002"}));
        ...
        //assert.commandWorked(admin.runCommand({split: "testDB.foo",manualsplit:true,find :{a : 30}}));
		assert.commandWorked(admin.runCommand({split: "testDB.foo",middle :{a : 30}}));
		var max=[];
		var min=[];
		for (var n=0;n<=3;n++){
		var chunks = cfg.chunks.find().toArray();
		...
		assert.writeOK(coll.insert({a: 31,c: 1001}));
        assert.commandWorked(admin.runCommand({shardCollection:"testDB.foo1",key:{b:1}}));
        assert.writeOK(coll1.insert({b: 10, d: 20}));
        assert.writeOK(coll.update({a: -1},{$set : {c : 1003}}, false,true));
        assert.writeOK(coll.update({a: 100},{$set : {c : 1004}}, false,true));
