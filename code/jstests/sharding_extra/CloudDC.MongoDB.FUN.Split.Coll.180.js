(function() {
	...
	var bulk = coll.initializeUnorderedBulkOp();
		...
		assert.commandWorked(admin.runCommand({split: "testDB.foo",find :{a : NumberLong(100)}}));
		
		assert.writeOK(coll.insert({a: NumberLong(-1), "name1": 20}));
		assert.writeOK(coll.insert({a: NumberLong(400), "name1": 20}));
        printShardingStatus(st.config,false);
