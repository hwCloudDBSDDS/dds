(function() {
	...
	assert.eq(num,51);
        ...
        assert.gte(shard1,50,"balance failed");
        ...
        sleep (5 * 1000);
		assert.commandWorked(admin.runCommand({moveChunk:"testDB.foo",find:{a: -10},to:"shard0000"}));
        ...
        assert.gte(shard1,27,"balance failed");
        assert.gte(shard2,27,"balance failed");
        assert.gte(shard3,27,"balance failed");
		printShardingStatus(st.config,false);
