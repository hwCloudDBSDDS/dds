(function() {
 ...
 assert.commandWorked(admin.runCommand({shardCollection:"testDB.foo",key:{a:"hashed"},unique:false}));
 ...
 for (var i =0; i < 1000; i++) {
     var floatn=i;
     assert.writeOK(coll.insert({a: floatn, s: bigString}));
 }
 ...
 for (var i=1;i<5;i++){
     ...
     var key="name"+i
     doc[key]=1
     assert.commandWorked(coll.ensureIndex(doc,{background:true}));
 }
 printShardingStatus(st.config,false);
 var array=[];
 for (var j=0;j<1;){

     assert.commandWorked(admin.runCommand({split: "testDB.foo", middle:{a : 750}}));

     assert.writeOK(coll.insert({a: -1, "name1": 20}));
     assert.writeOK(coll.insert({a: 400, "name1": 20}));
     printShardingStatus(st.config,false);
