(function() {
 ...
 for (var j=0;j<601;j++){
     ...
     sleep (1*1000);
     jsTest.log("waiting...");
     assert.lt(j,600,"Timeout");
 }
 for (var j=0;j<10000;j++){
     //assert.writeOK(coll1.insert({a: (-1-j),b: (-1-j)}));	
     coll1.insert({a: (-1-j),b: (-1-j)},{ writeConcern: { w: 1,j:true}});
 }
 //判断chunk增加
 sleep (20*1000);
 ...
 assert.eq(12000,coll.find().itcount());
 assert.eq(12000,coll1.find().itcount());
