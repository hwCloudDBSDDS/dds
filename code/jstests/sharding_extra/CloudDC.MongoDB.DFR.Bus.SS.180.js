(function() {
	...
	assert.commandWorked(testdb.createCollection("fo"));
		...
		assert.writeOK(testdb.fo.insert({a: -10, c: 10},{ writeConcern: { w: 1,j:true}}));
	var chunks = cfg.chunks.find().toArray();
	var datadases = cfg.databases.find().toArray();
	var num = cfg.chunks.find().itcount();
	assert.eq(num,52);
	for (var i = 0; i < num; i++) {
		var ns=chunks[i].ns;
	...
	assert.eq(chunks[j].shard,datadases[0].primary)
	...
	assert.eq(num,52);
		...
		assert.eq(chunks[m].shard,datadases[0].primary)
