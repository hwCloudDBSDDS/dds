(function() {
    'use strict';
		...
		var bigString = "";
		for (var i =0; i < 300; i++) {
	    var floatn=NumberDecimal(-i-0.1)
    ...
    assert.writeOK(bulk.execute());
		...
		assert.commandWorked(admin.runCommand({split: "testDB.foo",find :{a : NumberDecimal(100.1)}}));
		assert.commandWorked(admin.runCommand({split: "testDB.foo",bounds :[{a : "a"},{a:"z"}]}));
		assert.writeOK(coll.insert({a: NumberDecimal(-151.1), "name1": 20}));
		assert.writeOK(coll.insert({a: NumberDecimal(-149.1), "name1": 20}));
        printShardingStatus(st.config,false);
        ...
        assert.writeOK(coll.update({a:NumberDecimal(-1.1)},{"$set":{b:NumberDecimal(-1)}},false,true));
        assert.eq(902, coll.find().itcount());
		assert.eq(2, coll.find({"name1":20}).itcount());
		assert.eq(1, coll.find({b:NumberDecimal(-1)}).itcount());
