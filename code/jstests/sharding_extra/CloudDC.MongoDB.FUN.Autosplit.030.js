(function() {
 ...
 assert.commandWorked(admin.runCommand({shardCollection:"testDB.foo1",key:{a:"hashed"},numInitialChunks:3}));
 assert.commandWorked(coll.createIndex({b: 1}));
 assert.commandWorked(coll1.createIndex({b: 1}));
 //3个chunk内固定为200M,3-10个默认为2.5G，10个以上默认为5G
 ...
 assert.eq(1000,coll1.find().itcount());
