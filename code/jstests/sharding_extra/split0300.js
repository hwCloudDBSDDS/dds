(function() {
    'use strict';
	 ...
	 var ran=String("a"+ransp);
        ...
        //assert.commandFailed(admin.runCommand({split: "testDB.foo", manualsplit:true,find :{ransp : ransp,ran:ransp}}));
		assert.commandFailed(admin.runCommand({split: "testDB.foo",middle :{ransp : ransp,ran:ransp}}));
        printShardingStatus(st.config,false);
        ...
        coll.update({"a":2900,"b":123},{"$set":{"value":20}},{upsert:true});
