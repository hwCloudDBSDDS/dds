...
(function() {
    ...
    assert.commandWorked(db.adminCommand(
        {shardcollection: dbname + "." + coll, key: {a: "hashed"}, numInitialChunks: 150}));

    s.printShardingStatus();

    var numChunks = s.config.chunks.count();
    assert.eq(numChunks, 150, "should be exactly 150 chunks");
    ...
    s.config.shards.find().forEach(
        ...
        function(shard) {
            ...
            assert.gte(numChunksOnShard, Math.floor(150 / 3));
        });
