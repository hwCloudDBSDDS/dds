...
enum class KeyRangeCheckResult{
  //  Chunk cannot contain any record with a key in checked range
  RangeUnknown = 0,

  //  Chunk cannot contain any record with a key in checked range
  RangeSeparated,

  //  All keys within a range are allowed for a chunk
  RangeContained,

  //  Chunk may contain keys from some subrange of the checked range,
  //  while keys from some other subrange of this range are disallowed
  RangeOverlap

};

class IKeyRangeChecker {
public:
    ...
    //
    virtual bool DoesRecordBelongToChunk(const rocksdb::Slice& key, const rocksdb::Slice& value) const = 0;
    //
    ...
    //
    virtual KeyRangeCheckResult DoesKeyRangeBelongToChunk(const rocksdb::Slice& keyLow, const rocksdb::Slice& keyHigh) const = 0;

    virtual ~IKeyRangeChecker() {}
};
