...
class VersionBuilder::Rep {
 ...
 public:
  ...
  void CheckRange(const ImmutableCFOptions* ioptions) {
    if (nullptr == ioptions->range_checker){
      return;
    }
    Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
         "Version builder checker(%p)", ioptions->range_checker);
    FileMetaData* f = nullptr;
    size_t size = 0;
    uint64_t fnumber = 0;
    for (int level = 0; level < base_vstorage_->num_levels(); level++) {
      if( 0 == levels_[level].added_files.size()) {
        continue;
      }
      std::vector<FileMetaData*> delete_files;
      delete_files.reserve(levels_[level].added_files.size());
      size = levels_[level].added_files.size();
      for (const auto& pair : levels_[level].added_files) {
        f = pair.second;
        if(KeyRangeCheckResult::RangeSeparated
            == ioptions->range_checker->DoesKeyRangeBelongToChunk(f->smallest.user_key(),
                                                                  f->largest.user_key())){
