...
// split-feature: filter for memtable optimize
void MemTableList::SetFilter(){
	for (auto& m : current_->memlist_){
      m->filter_enable_ = true;  
	}
  
	for (auto& m_h : current_->memlist_history_){
      m_h->filter_enable_ = true;  
	}
  return ;
}
...
Status MemTableList::InstallMemtableFlushResults(
    ...
    SequenceNumber& max_encoded_seq) { // split-feature: support split DB
  AutoThreadOperationStageUpdater stage_updater(
  ...
  while (s.ok()) {
    ...
    if (batch_count > 0) {
      ...
      if (s.ok()) {         // commit new state
        ...
        // get the max sequence in this LogAndApply()
        for (auto& e : edit_list) {
          if(e->GetLastEncodedSequence() > max_encoded_seq) {
             max_encoded_seq = e->GetLastEncodedSequence();
          }
        }
