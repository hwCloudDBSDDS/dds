...
int InternalStats::DumpCFMapStats(
  ...
  for (int level = 0; level < number_levels_; level++) {
    ...
    if (comp_stats_[level].micros > 0 || files > 0) {
      ...
      total_file_size += vstorage->NumLevelBytesForSplit(cfd_->ioptions()->range_checker, level);
      double w_amp =
      ...
      PrepareLevelStats(&level_stats, files, files_being_compacted[level],
                        static_cast<double>(vstorage->NumLevelBytesForSplit(cfd_->ioptions()->range_checker, level)),
