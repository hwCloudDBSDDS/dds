...
#ifndef NDEBUG  // split-feature: support split DB
  //have been checked before, do not need to check again
  // This function is used to check the file belong to this DB instance,
  // because the range of DB instance is changed by split
  FdWithKeyRange* RangeFilterCheck() {
    KeyRangeCheckResult result = KeyRangeCheckResult::RangeUnknown;
    while (curr_index_in_curr_level_ < curr_file_level_->num_files) {
      FdWithKeyRange* file_info = &curr_file_level_->files[curr_index_in_curr_level_];
      result = files_[curr_level_][curr_index_in_curr_level_]->fd.range_check_result;
      if (result != KeyRangeCheckResult::RangeUnknown) {
        if (result == KeyRangeCheckResult::RangeSeparated) {
      ...
      result = range_checker_->DoesKeyRangeBelongToChunk(ExtractUserKey(file_info->smallest_key),
                                                         ExtractUserKey(file_info->largest_key));
      files_[curr_level_][curr_index_in_curr_level_]->fd.range_check_result = result;
      if (result != KeyRangeCheckResult::RangeSeparated) {
        return file_info;
      }
      ++curr_index_in_curr_level_;
...
#endif
  const IKeyRangeChecker* range_checker_;
...
// Confirm if the file belongs to this chunk by checking the key range of the file.
inline bool CheckFileKeyRange(
        const IKeyRangeChecker* range_checker, const FileMetaData* file_meta) {
  if (range_checker == nullptr) {
    return true;
  }
  return (KeyRangeCheckResult::RangeSeparated
            != range_checker->DoesKeyRangeBelongToChunk(file_meta->smallest.user_key(),
                                                        file_meta->largest.user_key()));
}

// Gets the total size of the sst files at all the other levels except the level 0.
void VersionStorageInfo::GetTotalSstFilesSizeAndRecNumExceptLevel0(
        const IKeyRangeChecker* range_checker,
        uint64_t& total_files_size,
        uint64_t& total_record_num) const {
  total_files_size = 0;
  total_record_num = 0;
  for (int level = 1; level < num_levels_; level++) {
    for (const auto& file_meta : LevelFiles(level)) {
      if (!CheckFileKeyRange(range_checker, file_meta)) {
        // Ignore files with no key belongs to this chunk.
        continue;
      }
      total_files_size += file_meta->fd.GetFileSize();
      if (file_meta->num_entries >= file_meta->num_deletions) {
          total_record_num += (file_meta->num_entries - file_meta->num_deletions);
      }
    }
  }
  return;
...
// Save the start loop position of each non empty level into the the temporary vector and
// find the smallest level at the same time, then move the smallest level to the front of
// the temporary vector.
void VersionStorageInfo::InitLevelsLoopPositionVector(
        std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos) {
  std::vector<FileMetaData*>::iterator file_meta_data_it;
  int smallest_level = -1;
  const int start_level = 1;
  const int levels_num = num_levels();
  for (int level = start_level; level < levels_num; level++) {
    file_meta_data_it = files_[level].begin();
    if (file_meta_data_it != files_[level].end()) {
      levels_loop_pos.push_back(std::pair<int,std::vector<FileMetaData*>::iterator>(level,file_meta_data_it));
      if ((0 > smallest_level)
          || (0 > InternalComparator()->Compare((*file_meta_data_it)->largest,
                                                (*(levels_loop_pos[smallest_level].second))->largest))) {
        smallest_level = levels_loop_pos.size() - 1;
      }
    }
  }

  // Always keep the current smallest level at the front of the temporary vector.
  if (smallest_level > 0) {
    std::swap(levels_loop_pos[smallest_level], levels_loop_pos[0]);
  }
}

// Find the sub smallest level in the temporary vector.
int VersionStorageInfo::FindSubSmallestLevel(
        const std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos) {
  // Find the sub smallest level after the smallest one always kept at the front.
  uint32_t start_index = 1;
  uint32_t end_index = levels_loop_pos.size();
  int smallest_level = -1;
  for (uint32_t i = start_index; i < end_index; i++) {
    if ((0 > smallest_level)
        || (0 > InternalComparator()->Compare((*(levels_loop_pos[i].second))->largest,
                                              (*(levels_loop_pos[smallest_level].second))->largest))) {
      smallest_level = i;
    }
  }
...
// Loop all the files that belong to the current smallest level and are smaller than the sub smallest level.
// The current smallest level must not be the last one, so needn't to check total_files_size.
Status VersionStorageInfo::LoopCurrentSmallestLevel(
        const IKeyRangeChecker* range_checker,
        std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos,
        const uint64_t half_files_size,
        std::string& split_point,
        uint64_t& left_datasize,
        uint64_t& left_numrecord) {
  // Find the sub smallest level and get the first file of it.
  int sub_smallest_level = FindSubSmallestLevel(levels_loop_pos);
  FileMetaData* sub_file_meta_data = *(levels_loop_pos[sub_smallest_level].second);

  // Get the current loop position of the smallest level.
  int level = levels_loop_pos[0].first;
  std::vector<FileMetaData*>::iterator file_meta_data_it = levels_loop_pos[0].second;

  // Loop current smallest files at current smallest level until the sub smallest level becomes the smallest one.
  while ((file_meta_data_it != files_[level].end())
         && (InternalComparator()->Compare((*file_meta_data_it)->largest, sub_file_meta_data->largest) <= 0)) {
    if (!CheckFileKeyRange(range_checker, *file_meta_data_it)) {
      // Ignore files with no key belongs to this chunk.
      file_meta_data_it++;
      continue;
    }
  ...
  // Update the loop position of the old smallest level.
  levels_loop_pos[0].second = file_meta_data_it;

  // The old sub smallest level must be the new smallest level, move it to the front.
  std::swap(levels_loop_pos[0], levels_loop_pos[sub_smallest_level]);

  // Erase the old smallest level if it has been traversed.
  if (file_meta_data_it == files_[level].end()) {
    levels_loop_pos.erase(levels_loop_pos.begin() + sub_smallest_level);
  }

  // Need to go on loop the new smallest level.
  return Status::NotFound();
}

// Loop the remained files at the last remained level.
Status VersionStorageInfo::LoopTheLastLevel(
        const IKeyRangeChecker* range_checker,
        std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos,
        const uint64_t total_files_size,
        std::string& split_point,
        uint64_t& left_datasize,
        uint64_t& left_numrecord) {
  uint64_t half_files_size = total_files_size / 2;
  std::vector<FileMetaData*>::iterator file_meta_data_it;
  int level = 0;

  if (levels_loop_pos.size() != 1) {
    // Forever can not go to here.
    return Status::NotFound();
  }

  level = levels_loop_pos[0].first;
  for (file_meta_data_it = levels_loop_pos[0].second;
       file_meta_data_it != files_[level].end();
       file_meta_data_it++) {
    if (!CheckFileKeyRange(range_checker, *file_meta_data_it)) {
      // Ignore files with no key belongs to this chunk.
      continue;
...
Status VersionStorageInfo::LoopLevelsFileInOrder(
        const IKeyRangeChecker* range_checker,
        std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos,
        const uint64_t total_files_size,
        std::string& split_point,
  ...
  while (levels_loop_pos.size() > 1) {
    // Loop the current smallest level and get the new smallest level.
    s = LoopCurrentSmallestLevel(range_checker, levels_loop_pos, half_files_size,
                                 split_point, left_datasize, left_numrecord);
    if (!s.IsNotFound()) {
      // Result has been made.
      return s;
    }

    // Go on looping the new smallest level.
  }

  // Loop the remained files at the last remained level.
  return LoopTheLastLevel(range_checker, levels_loop_pos, total_files_size,
                          split_point, left_datasize, left_numrecord);
}

// Get the split point that halves the total size of the sst files at all the other levels except level 0.
Status VersionStorageInfo::GetSplitPointBySize(
        const IKeyRangeChecker* range_checker,
        std::string& split_point,
        uint64_t& left_datasize,
        uint64_t& left_numrecord,
  ...
  // Each element is a pair including the level and the iterator to the first file at the level.
  // with the level index indicated by the array index.
  std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>> levels_loop_pos;
  uint64_t total_files_size = 0;
  uint64_t total_record_num = 0;
  Status s;

  // Init the value of output parameters.
  split_point = "";
  left_datasize = 0;
  left_numrecord = 0;
  right_datasize = 0;
  ...
  // find the smallest level at the same time, then move the smallest level to the front of
  // the temporary vector.
  InitLevelsLoopPositionVector(levels_loop_pos);

  // Loop all files at all levels in the order of the largest key and accumulate the size.
  s = LoopLevelsFileInOrder(range_checker, levels_loop_pos, total_files_size,
                            split_point, left_datasize, left_numrecord);
  if (s.ok()) {
    right_datasize = total_files_size - left_datasize;
    right_numrecord = total_record_num - left_numrecord;
  } else {
    split_point = "";
    left_datasize = 0;
    left_numrecord = 0;
...
// Also level 0 will be ignored.
Status VersionStorageInfo::GetSplitInfoBySplitPoint(
        const IKeyRangeChecker* range_checker,
        ...
        uint64_t& right_datasize,
        uint64_t& right_numrecord) {
  InternalKey split_point_key(Slice(split_point), 0, kTypeValue);
  std::vector<FileMetaData*>::iterator file_meta_data_it;
  int level = 0;
  const int start_level = 1; // except level 0
  const int levels_num = num_levels();
  uint64_t total_files_size = 0;
  uint64_t total_record_num = 0;
  ...
  // Accumulating the approximate size and record number of the left db.
  for (level = start_level; level < levels_num; level++) {
    for (file_meta_data_it = files_[level].begin();
         file_meta_data_it != files_[level].end();
         file_meta_data_it++) {
      if (0 < InternalComparator()->Compare((*file_meta_data_it)->largest,split_point_key)) {
        break;
      }
      if (!CheckFileKeyRange(range_checker, *file_meta_data_it)) {
        // Ignore files with no key belongs to this chunk.
  ...
  // Get the approximate size and record number of the right db.
  right_datasize = total_files_size - left_datasize;
  right_numrecord = total_record_num - left_numrecord;
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  GetContext get_context(
      ...
      merge_operator_ ? &pinned_iters_mgr : nullptr, vset_->db_options_->info_log);
  ...
  uint16_t get_file_times = 0;
  FilePicker fp(
      storage_info_.files_, user_key, ikey, &storage_info_.level_files_brief_,
      storage_info_.num_non_empty_levels_, &storage_info_.file_indexer_,
      user_comparator(), internal_comparator(),
      cfd_->ioptions()->range_checker);// split-feature: support split DB
...
uint64_t VersionStorageInfo::NumLevelBytesForSplit(const IKeyRangeChecker* range_checker, int level) const {
  // When calculate the total size of a specific level, we should remove the files
  // that do not belong to the current db after split
  uint64_t sum = 0;
  for (size_t i = 0; i < files_[level].size() && files_[level][i]; i++) {
    if(!CheckFileKeyRange(range_checker, files_[level][i])) {
      // Ignore files with no key belongs to this chunk
      continue;
    }
    sum += files_[level][i]->fd.GetFileSize();
  }
  return sum;
}
...
#ifndef NDEBUG  //have been checked before, do not need to check again
// Clear shared flag of all files.
void VersionSet::ClearSharedFlag() {
  Version* dummy_versions = nullptr;
  Version* v = nullptr;
  ...
  for (auto cfd : *GetColumnFamilySet()) { //Loops over different column families
    if (cfd->IsDropped()) {
      continue;
    }

    dummy_versions = cfd->dummy_versions();
    for (v = dummy_versions->next_;
         v != dummy_versions;
         v = v->next_) { //Loops over different versions
      files = v->storage_info_.files_;
      num_levels = v->storage_info_.num_non_empty_levels_;
      curr_level = 0;
      while (curr_level < num_levels) { // Loops over different levels.
        //loop all files of current level
        curr_file_level = &(v->storage_info_.level_files_brief_)[curr_level];
        curr_index = 0;
        while (curr_index < curr_file_level->num_files) { // Loops over different files
          files[curr_level][curr_index]->fd.range_check_result = KeyRangeCheckResult::RangeUnknown;
          ++curr_index;
        }
        curr_level++;
      }
...
Status VersionSet::LogAndApply(ColumnFamilyData* column_family_data,
  ...
  // split-feature:  when Db is splitting , create new manifest is not allowed
  if (!descriptor_log_ ||
      (!IsDbSplitting() &&
  ...
  // because &w is ensuring that all new writes get queued.
  {
    ...
    if (new_descriptor_log) {
      ...
      if (s.ok()) {
        ...
        std::vector<const IKeyRangeChecker*> range_checker_list;
        range_checker_list.clear();
        s = WriteSnapshot(descriptor_log_.get(),range_checker_list,false);
      }
    }
    ...
    if (s.ok()) {
      for (auto& e : batch_edits) {
        ...
        if (IsDbSplitting()){
          s1 =descriptor_log_child_->AddRecord(record);
          if (!s1.ok()) {
            Log(InfoLogLevel::ERROR_LEVEL, db_options_->info_log,
                "Add version edit to manifest failed\n");
            break;
          }
...
Status VersionSet::Recover(
  ...
  if (s.ok()) {
    for (auto cfd : *column_family_set_) {
      ...
      // split-feature: support split DB
      // check if files belong to db. when file not belong to db,
      // remove file from levels_[level].added_files
      Log(InfoLogLevel::INFO_LEVEL, db_options_->info_log,
          "go through Column family [%s] (ID %u)",cfd->GetName().c_str(), cfd->GetID());

      builder->CheckRange(cfd->ioptions());
...
Status VersionSet::WriteSnapshot(log::Writer* log,
                  // split-feature: support split DB
                  const std::vector<const IKeyRangeChecker*>& right_db_range_checker,
                  const bool call_in_splitting) {
  ...
  for (auto cfd : *column_family_set_) {
    if (cfd->IsDropped()) {
      Log(InfoLogLevel::INFO_LEVEL, db_options_->info_log,
          "WriteSnapshot: drop Column family [%s] ",cfd->GetName().c_str());
      continue;
    }
    {
      ...
      if (!edit.EncodeTo(&record)) {
        return Status::Corruption(
            "WriteSnapshot:Unable to Encode Comparator VersionEdit:" + edit.DebugString(true));
      }
...

    {
      ...
      edit.SetColumnFamily(cfd->GetID());
      // split-feature: support split DB
      if(!call_in_splitting){
        checker = cfd->ioptions()->range_checker;
      }
      else{
        if(right_db_range_checker.size() > 0) {
          for (auto iter = right_db_range_checker.begin(); iter != right_db_range_checker.end(); ++iter){
            if(0 == strcmp((*iter)->GetName(),cfd->GetName().c_str())){
                checker = (*iter);
                break;
            }
          }
      ...
      for (int level = 0; level < cfd->NumberLevels(); level++) {
        for (const auto& f :
          ...
          // split-feature: support split DB
          // filter out SST file which not belong to db
          if(nullptr != checker &&
...
// split-feature: support split DB
// write snapshot for the right side
Status VersionSet::LogAndApply4Split(const std::string & child_dbname,
  ...
  Log(InfoLogLevel::INFO_LEVEL, db_options_->info_log,
      "write manifest snapshort %" PRIu64 " for the child DB\n", manifest_file_number_);

  
  //write snapshot about the child DB instance
  unique_ptr<WritableFile> descriptor_file;
  EnvOptions opt_env_opts = env_->OptimizeForManifestWrite(env_options_);
  s = NewWritableFile(
      env_, DescriptorFileName(child_dbname, manifest_file_number_),
      &descriptor_file, opt_env_opts);
  if (s.ok()) {
    descriptor_file->SetPreallocationBlockSize(
        db_options_->manifest_preallocation_size);
    ...
    descriptor_log_child_.reset(
        new log::Writer(std::move(file_writer), 0, false));
    s = WriteSnapshot(descriptor_log_child_.get(), right_db_range_checker, true);
  }
  
  if(!s.ok()){
    Log(InfoLogLevel::ERROR_LEVEL, db_options_->info_log,
        "write child manifest snapshort %" PRIu64 " failed\n", manifest_file_number_);
  }
  //Update the child db current file
  s = SetCurrentFile(env_, child_dbname, manifest_file_number_, nullptr); 
  if(!s.ok()){
   Log(InfoLogLevel::ERROR_LEVEL, db_options_->info_log,
...
Status VersionSet::StopWriteSplitDeta(ColumnFamilyData* column_family_data, 
                                      InstrumentedMutex* mu){
  Status s;
  VersionEdit dummy_edit;
  autovector<VersionEdit*> edit_list;
