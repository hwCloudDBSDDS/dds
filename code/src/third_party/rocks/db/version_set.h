...
class VersionStorageInfo {
 public:
  ...
  // Return the combined file size of all files at the specified level, but does not include files that belong to this chunk
  uint64_t NumLevelBytesForSplit(const IKeyRangeChecker* range_checker, int level) const;
  ...
  Status GetSplitPointBySize(const IKeyRangeChecker* range_checker,
   ...
   Status GetSplitInfoBySplitPoint(const IKeyRangeChecker* range_checker,
 ...
 private:
  ...
  // Called by GetSplitPointBySize().
  void GetTotalSstFilesSizeAndRecNumExceptLevel0(
          const IKeyRangeChecker* range_checker,
          uint64_t& total_files_size,
          uint64_t& total_record_num) const;

// The following functions are called by GetSplitPointBySize():
  // Prepare for the loop process: Save the start loop position of each non empty level into
  // the the temporary vector and find the smallest level at the same time, then move the
  // smallest level to the front of the temporary vector.
  void InitLevelsLoopPositionVector(
  ...
  Status LoopCurrentSmallestLevel(
          const IKeyRangeChecker* range_checker,
          std::vector<std::pair<int,std::vector<FileMetaData*>::iterator>>& levels_loop_pos,
...
class VersionSet {
 public:
  ...
  Status LogAndApply4Split(const std::string & child_dbname,
      ColumnFamilyData* column_family_data,
      InstrumentedMutex* mu,
      const std::vector<const IKeyRangeChecker*>& right_db_range_checker);

  Status StopWriteSplitDeta(ColumnFamilyData* column_family_data,
      InstrumentedMutex* mu);
 ...
 private:
  ...
  Status WriteSnapshot(log::Writer* log,
                  // split-feature: support split DB
                  const std::vector<const IKeyRangeChecker*>& right_db_range_checker,
                  const bool call_in_splitting = false);
  ...
  unique_ptr<log::Writer> descriptor_log_child_;
