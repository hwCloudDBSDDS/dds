...
class DBImpl : public DB {
 public:
  ...
  Status PrepareSplitDb(Env* right_env, const std::vector<const IKeyRangeChecker*>& right_db_range_checker,
                        const DbInstanceOption& option) override;
  ...
  Status RollbackSplit(Env* right_env) override;
...
//  struct WriteContext;
  ...
  void GetCopyfiles(std::vector<std::string>& files);

  //filter for memtable enable
  void SetMemtableFilterEnable(void);
  ...
  std::atomic<uint64_t> max_sequence_split_;
  // Value when all memTables active before split are flushed
  static const uint64_t kAllKvsBeforeSplitAreFlushed  = port::kMaxUint64;

  // used to wait for the max_sequence_split_ kv is compeleted to flush
  // wal_flush_split_cv_.wait at DBImpl::FlushPartMemtable()
  // wal_flush_split_cv_.SignalAll at DBImpl::FlushMemTableToOutputFile()
  InstrumentedCondVar wal_flush_split_cv_;
