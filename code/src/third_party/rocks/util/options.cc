...
ColumnFamilyOptions::ColumnFamilyOptions()
    : comparator(BytewiseComparator()),
      ...
      // range_checker(std::shared_ptr<KeyRangeChecker>(new KeyRangeChecker))
      range_checker(nullptr){
...
ColumnFamilyOptions::ColumnFamilyOptions(const Options& options)
    : comparator(options.comparator),
      ...
      // split-feature: support split DB
      range_checker(options.range_checker){
  assert(memtable_factory.get() != nullptr);
...
void ColumnFamilyOptions::Dump(Logger* log) const {
  ...
  Header(log, " Options.max_write_buffer_number: %d", max_write_buffer_number);
    ...
    // split-feature: support split db
    Header(log, "       Options.range_checker: %s",
      range_checker ? range_checker->GetName() : "None");
