...
namespace mongo
{
...

        {
                      ...
                      ShardServerState::kActive)))
            {
                return Status(ErrorCodes::BadValue,
                              str::stream() << "Invalid shard server state value: " << shardServerState);
            }
            ...
            else if (status == ErrorCodes::NoSuchKey)
            {
                if (shardServer._state != ShardServerState::kStandby)
                {
                    return Status(ErrorCodes::NoSuchKey,
                                  str::stream() << "empty shard id when shard server is active or toBeActive");
                }
                else
                {
                    // shardName field should not be set if active == false
    ...
    BSONObj ShardServerType::toBSON() const
    {
        ...
        if (_state)
        { builder.append(state(), static_cast<std::underlying_type<ShardServerState>::type>(getState())); }
