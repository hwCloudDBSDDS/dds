 ...
 * command.
 */
class ActiveReadyRequest {
public:
    ActiveReadyRequest(NamespaceString nss,
     ...
     * it contains the correct types, constructs a ActiveReadyRequest object from it.
     *
     * {
     ...
     * }
     */
    static StatusWith<ActiveReadyRequest> parseFromConfigCommand(const BSONObj& cmdObj);

    /**
     * Creates a BSONObjBuilder and uses it to create and return a BSONObj from this
     * ActiveReadyRequest instance. Calls appendAsConfigCommand and tracks on the passed-in
     * writeConcern.
     */
...
private:
    /**
     * Returns a validation Status for this ActiveReadyRequest instance. Performs checks for
    ...
    std::string _shardName; // store shard name which is generated in the registration process
    std::string _processIdentity; // shard process identity generated when process starts
};
