...
TEST(CfgRegisterShardsvrRequest, BasicValidConfigCommand) {
    auto request = assertGet(RegShardSvrRequest::parseFromConfigCommand(
        BSON(kConfigsvrRegShardSvr << "TestRegisterShardSvr.TestConfigCommand" << kHostAndPort
    ...
    ASSERT_EQ(assertGet(ConnectionString::parse("test0:1234")).toString(),
...
TEST(CfgRegisterShardsvrRequest, MissingNameSpaceErrors) {
    auto request = RegShardSvrRequest::parseFromConfigCommand(
        BSON(kHostAndPort << ConnectionString(HostAndPort("test0:1234")).toString()));
...
TEST(CfgRegisterShardsvrRequest, MissingConnectionErrors) {
    auto request = RegShardSvrRequest::parseFromConfigCommand(
        BSON(kConfigsvrRegShardSvr << "TestRegisterShardSvr.TestConfigCommand"));
...
TEST(CfgRegisterShardsvrRequest, WrongNameSpaceTypeErrors) {
    auto request = RegShardSvrRequest::parseFromConfigCommand(
        BSON(kConfigsvrRegShardSvr << 1234 << kHostAndPort
                                   << ConnectionString(HostAndPort("test0:1234")).toString()));
...
TEST(CfgRegisterShardsvrRequest, WrongConnectionTypeErrors) {
    auto request = RegShardSvrRequest::parseFromConfigCommand(BSON(
        kConfigsvrRegShardSvr << "TestRegisterShardSvr.TestConfigCommand" << kHostAndPort << 1234));
