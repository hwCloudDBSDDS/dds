...
ProcessStageTime::ProcessStageTime(std::string processName):
...
BSONObj ProcessStageTime::toBSON() {
    ...
    processBSON.append("processDuration", 
        durationCount<Milliseconds>(getProcessDuration())); 
    ...
    for (i = 0; i < _stages.size(); i++) {
        if (i < _stages.size() - 1) {
            stageDurationBSON.append(_stages[i].name, durationCount<Milliseconds>(
                _stages[i+1].startTime - _stages[i].startTime));
        }
    }    
