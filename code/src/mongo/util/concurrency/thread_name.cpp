...
void setThreadName(StringData name) {
    ...
    std::string* pthreadName = threadName.get();
    ...
    setWindowsThreadName(GetCurrentThreadId(), pthreadName->c_str());
...
/* END zhangchengwei 00224071 comment */
    ...
    if (getpid() != syscall(SYS_gettid)) {
        ...
        int error = 0;
        if (pthreadName->size() > 15) {
            std::string shortName = str::stream() << pthreadName->substr(0, 7) << '.'
                                                  << pthreadName->substr(pthreadName->size() - 7);
            error = pthread_setname_np(pthread_self(), shortName.c_str());
        } else {
            error = pthread_setname_np(pthread_self(), pthreadName->c_str());
        }
