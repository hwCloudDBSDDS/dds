...
#include "mongo/db/modules/rocks/src/Chunk/ChunkRocksDBInstance.h"
...
void Database::clearTmpCollections(OperationContext* txn) {
    ...
    for (list<string>::iterator i = collections.begin(); i != collections.end(); ++i) {
        string ns = *i;
        // invariant(NamespaceString::normal(ns)); //chunk collection include '$' will be thougt as
        // virtual collection.
...
void Database::getStats(OperationContext* opCtx, BSONObjBuilder* output, double scale) {
    ...
    list<string> collectionsWithoutChunkId;
    for (list<string>::const_iterator it = collections.begin(); it != collections.end(); ++it) {
        const string ns = *it;
        ...
        string nsWihoutChunkId = ns;
        if (pos != std::string::npos) {
            nsWihoutChunkId = ns.substr(0, pos + 1);
        }
        if (collectionsWithoutChunkId.end() == std::find(collectionsWithoutChunkId.begin(),
                                                         collectionsWithoutChunkId.end(),
                                                         nsWihoutChunkId)) {
            nCollections += 1;
            indexes += collection->getIndexCatalog()->numIndexesTotal(opCtx);
            collectionsWithoutChunkId.push_back(nsWihoutChunkId);
        }
...
Status Database::dropCollection(OperationContext* txn, StringData fullns) {
    ...
    Collection* collection = getCollection(fullns, true);
    if (!collection) {
    ...
    if (collection) {
        ...
        if (collection->getState() == Collection::ChunkState::ASSIGNING) {
            index_LOG(1) << "wait chunk to be assigned completely:" << fullns;
...
Collection* Database::getCollection(StringData ns, bool including_all) {
    ...
    if (it != _collections.end() && it->second) {
        ...
        if (!including_all && !it->second->isActive()) {
            index_warning() << "coll is not active " << ns << " "
...
void Database::listCollectionNSs(std::vector<NamespaceString>& out) const {
    for (const auto& entry : _collections) {
        if (entry.second->isActive() &&
            entry.second->getDbInstanceType() == Collection::dbInstanceType::kUserDb) {
...
void Database::_checkCanCreateCollection(const NamespaceString& nss,
    ...
    size_t nsSize = nss.ns().find("$");
    if (nsSize == std::string::npos) {
        nsSize = nss.size();
    }
    uassert(17381,
            str::stream() << "fully qualified namespace " << nss.ns() << " is too long "
                          ...
                          << " bytes)",
            !nss.isNormal() || nsSize <= NamespaceString::MaxNsCollectionLen);
...
Collection* Database::createCollection(OperationContext* txn,
    ...
    NamespaceString nss(ns);
    index_log() << "[createCollection] Database::createCollection ns: " << ns;
...
Status Database::mockAssignChunk(OperationContext* txn,
                                 ...
                                 const BSONObj& idIndex) {
    index_log() "[assignChunk] Database::mockAssign ns: " << ns;
    ...
    {
        stdx::lock_guard<stdx::mutex> lk(_lockMockAssign);
        ...
        if (createDefaultIndexes && !idIndex.isEmpty()) {
            ...
            if (options.toBSON().hasField("collation") && indexVersion >= IndexDescriptor::IndexVersion::kV2) {
                std::string collation = options.toBSON().getField("collation").toString(false);
                index_log() << "collation : " << collation;
                b.append(options.toBSON().getField("collation"));
            }
            BSONArrayBuilder indexArrayBuilder;
    ...
    AssignChunkRequest::appendAsCommand(
        &builder, chunkType, collType, true, "shard0000", "22916_1509969868746_16963381");
...
Status Database::assignChunk(OperationContext* txn,
    ...
    if (collection) {
        ...
        if (!collection->isActive()) {
            index_LOG(1) << "retry until chunk to be assigned completely:" << ns;
            throw WriteConflictException();
        }
        return Status(ErrorCodes::NamespaceExists,
                      str::stream() << "a collection for chunk'" << ns.toString()
                                    << "' already exists");
        ;
    }
    ...
    if (!status.isOK()) {
        index_err() << "coll_Options.parse(cmdObj) is not ok, cmdObj:" << cmdObj;
        return status;
    }
    ...
    if (coll_Options.isView()) {
        index_LOG(1) << "Database::assignChunk()-> createView";
    ...
    getGlobalServiceContext()
        ...
        ->noteStageStart("createCollection");
    ...
    invariant(collection);
    collection->setState(Collection::ChunkState::ASSIGNING);
    collection->setDbInstanceType(Collection::dbInstanceType::kUserDb);
...
Status Database::assignChunkFinalize(OperationContext* txn,
                                     StringData ns,
                                     const AssignChunkRequest& assignChunkRequest) {
    Collection* collection = this->getCollection(ns, true);
    invariant(collection);
    ...
    try {
        ...
        _dbEntry->postInitRecordStore(txn, ns);
        ...
        if (!specsWithStatus.isOK()) {
            index_err() << "Database::assignChunkFinalize specsWithStatus is not ok";
            uassertStatusOK(specsWithStatus.getStatus());
        }

        std::vector<BSONObj> specs = std::move(specsWithStatus.getValue());
        ...
        if (ClusterRole::None != serverGlobalParams.clusterRole) {
            ...
            if (!indexSpecsWithDefaults.isOK()) {
                index_err() << "Database::assignChunkFinalize indexSpecsWithDefaults is not ok";
        ...
        // filter _id index
        bool createIdIndex =
            collection->requiresIdIndex() && (options.autoIndexId == CollectionOptions::YES ||
                                              options.autoIndexId == CollectionOptions::DEFAULT);
        ...
        if (specs.empty()) {
            index_warning() << "Database::assignChunkFinalize specs is empty";
            throw DBException("indexes specs is empty", ErrorCodes::InvalidOptions);
        }
        ...
        collection->setState(Collection::ChunkState::STABLE);
    } catch (const std::exception& exc) {
        // catch anything thrown within try block that derives from std::exception
        index_log() << "assignChunkFinalize failed for" << assignChunkRequest.getChunk().getName();

        collection->setState(Collection::ChunkState::CORRUPTED);
        return Status(ErrorCodes::RebalanceEventFailed, "failed to assign");
    }
...
Status Database::toUpdateChunkMetadata(OperationContext* txn, StringData ns, BSONArray& indexes) {
    ...
    NamespaceString nss(ns);
    if(!nss.isSystemCollection() && useMultiInstance){
