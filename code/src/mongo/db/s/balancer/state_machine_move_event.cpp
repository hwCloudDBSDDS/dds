...
MoveEvent::MoveEvent(const ChunkType& chunk,
                     ...
                     bool userCommand)
                     : IRebalanceEvent(StateMachine::EventType::kMoveEvent, result, userCommand) {
...
MoveEvent::MoveEvent(const std::string& eventId,
                     IRebalanceEvent::EventResultInfo* result)
                     : IRebalanceEvent(StateMachine::EventType::kMoveEvent, result) {
    _eventId = eventId;
}
...
Status MoveEvent::dataFromBSON(const BSONObj& eventData) {
    {
        ...
        _moveEventInfo.curState = static_cast<StateMachine::RebalanceEventState>(curstate);
    }

    {
        ...
        _moveEventInfo.prevState = static_cast<StateMachine::RebalanceEventState>(prevstate);
    }
...
void MoveEvent::transitState(StateMachine::RebalanceEventState nextState) {
...
StateMachine::RebalanceEventState MoveEvent::getState() {
    return _moveEventInfo.curState;
}

StateMachine::RebalanceEventState MoveEvent::getPrevState() {
...
void MoveEvent::refreshChunk(const ChunkType& pChunk,
