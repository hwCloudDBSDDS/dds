...
SplitEvent::SplitEvent(const ChunkType& parentChunk,
                       ...
                       bool needWriteEvent,
                       bool bgCommand)
    : IRebalanceEvent(BalanceEventEngine::EventType::kSplitEvent,
                      result,
                      userCommand,
                      needWriteEvent,
                      bgCommand) {
...
SplitEvent::SplitEvent(const std::string& eventId, IRebalanceEvent::EventResultInfo* result)
    : IRebalanceEvent(BalanceEventEngine::EventType::kSplitEvent, result) {
    _eventId = eventId;
}
...
Status SplitEvent::dataFromBSON(const BSONObj& eventData) {
    {
        ...
        _splitEventInfo.curState = static_cast<BalanceEventEngine::RebalanceEventState>(curstate);
    }

    {
        ...
        _splitEventInfo.prevState = static_cast<BalanceEventEngine::RebalanceEventState>(prevstate);
    }
    ...
    {
        BSONElement chunk;
        Status status = bsonExtractTypedField(eventData, kChildChunkFieldName, Object, &chunk);
        if (!status.isOK()) {
            return {status.code(), str::stream() << "Invalid childChunk data " << status.reason()};
        }
    ...
    {
        if (_splitEventInfo.curState >= BalanceEventEngine::RebalanceEventState::kCommitSplit) {
            BSONElement splitPoint;
            Status status =
                bsonExtractTypedField(eventData, kSplitPointFieldName, Object, &splitPoint);
            if (!status.isOK()) {
                return {status.code(),
...
void SplitEvent::transitState(BalanceEventEngine::RebalanceEventState nextState) {
...
const ShardIdent SplitEvent::getSource() const {
    auto shardId = _splitEventInfo.parentChunk.getShard();
    auto processIdent = _splitEventInfo.parentChunk.getProcessIdentity();
    return ShardIdent(shardId, processIdent);
}

BalanceEventEngine::RebalanceEventState SplitEvent::getState() {
    return _splitEventInfo.curState;
}

BalanceEventEngine::RebalanceEventState SplitEvent::getPrevState() {
...
/*
BalanceEventEngine::EventType SplitEvent::getEventType() {
    return BalanceEventEngine::EventType::kSplitEvent;
}*/
...
bool SplitEvent::shouldContinue() {
    if (_splitEventInfo.curState == BalanceEventEngine::RebalanceEventState::kAcknowledgeSplit ||
        _splitEventInfo.curState == BalanceEventEngine::RebalanceEventState::kCommitSplit) {
        return true;
    }
    return false;
}
...
void SplitEvent::refreshChunk(const ChunkType& pChunk, const ChunkType& cChunk) {
...
void SplitEvent::setRollbackFlag(bool flag) {
    _needRollback = flag;
}

bool SplitEvent::getRollbackFlag() {
    return _needRollback;
}

void SplitEvent::setSplitSuccessFlag(bool flag) {
    _splitSuccess = flag;
}

bool SplitEvent::getSplitSuccessFlag() {
