...
OffloadEvent::OffloadEvent(const ChunkType& chunk,
                           ...
                           bool needWriteEvent)
    : IRebalanceEvent(
          BalanceEventEngine::EventType::kOffloadEvent, result, userCommand, needWriteEvent) {
...
OffloadEvent::OffloadEvent(const std::string& eventId, IRebalanceEvent::EventResultInfo* result)
    : IRebalanceEvent(BalanceEventEngine::EventType::kOffloadEvent, result) {
    _eventId = eventId;
}
...
Status OffloadEvent::dataFromBSON(const BSONObj& eventData) {
    {
        ...
        _offloadEventInfo.curState = static_cast<BalanceEventEngine::RebalanceEventState>(curstate);
    }
...
void OffloadEvent::transitState(BalanceEventEngine::RebalanceEventState nextState) {
...
BalanceEventEngine::RebalanceEventState OffloadEvent::getState() {
    return _offloadEventInfo.curState;
}

BalanceEventEngine::RebalanceEventState OffloadEvent::getPrevState() {
...
void OffloadEvent::refreshChunk(const ChunkType& pChunk, const ChunkType& cChunk) {}
