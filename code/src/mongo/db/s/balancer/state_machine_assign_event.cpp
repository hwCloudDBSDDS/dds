...
AssignEvent::AssignEvent(const ChunkType& chunk,
                         ...
                         bool newChunk, bool userCommand)
                         : IRebalanceEvent(StateMachine::EventType::kAssignEvent, result, userCommand) {
...
AssignEvent::AssignEvent(const std::string& eventId,
                         IRebalanceEvent::EventResultInfo* result)
                         : IRebalanceEvent(StateMachine::EventType::kAssignEvent, result) {
    _eventId = eventId;
}
...
Status AssignEvent::dataFromBSON(const BSONObj& eventData) {
    {
        ...
        _assignEventInfo.curState = static_cast<StateMachine::RebalanceEventState>(curstate);
    }

    {
        ...
        _assignEventInfo.prevState = static_cast<StateMachine::RebalanceEventState>(prevstate);
    }
...

    {
        ...
        Status status = bsonExtractTypedField(eventData, kTargetShardFieldName, Object, &targetShard);
        if (!status.isOK())
            return status;
...
void AssignEvent::transitState(StateMachine::RebalanceEventState nextState) {
...
StateMachine::RebalanceEventState AssignEvent::getState() {
    return _assignEventInfo.curState;
}

StateMachine::RebalanceEventState AssignEvent::getPrevState() {
...
void AssignEvent::refreshChunk(const ChunkType& pChunk,
