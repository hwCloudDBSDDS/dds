...
MoreDbRocksRecoveryUnit::~MoreDbRocksRecoveryUnit()
{
    ...
    for (auto it=_moreTransaction.begin(); it!=_moreTransaction.end(); ++it) {
        delete (it->second);
    }
...
const rocksdb::Snapshot* MoreDbRocksRecoveryUnit::snapshot() 
{
    if (_readFromMajorityCommittedSnapshot) {
        log() << "MoreDbRocksRecoveryUnit::snapshot()-> faild not support readConcern:majority!";
...
rocksdb::Status MoreDbRocksRecoveryUnit::Get(const rocksdb::Slice& key, std::string* value, rocksdb::ColumnFamilyHandle* cf)
{
    cf = ResolveColumnFamily(cf);
    if (writeBatch()->GetWriteBatch()->Count() > 0) {
    ...
    for(auto it=_snapshots.begin(); it!=_snapshots.end(); ++it) {
        if (it->second) {
            ...
            if (_moreTransaction.find(it->first) != _moreTransaction.end()) {
                ...
                delete (_moreTransaction[it->first]);
...
void MoreDbRocksRecoveryUnit::_commit(OperationContext* txn)
{
    for(auto it=_writeBatchs.begin(); it!=_writeBatchs.end(); ++it) {
        ...
        if (wb->Count() != 0) {
            ...
            throwExceptionIfWriteBatchBelongsToChildSide(txn, it->second.get(), it->first, isSplitOnGoing);
            ...
            try{
                for (auto pair : _moreDBDeltaCounters) {
                    for (auto delta: pair.second)
                    {
                        ...
                        LOG(3)<<"MoreDbRocksRecoveryUnit::_commit counterKey:"<<delta.first
                          <<", counter._value:"<< counter._value->load(std::memory_order::memory_order_relaxed)
                          <<", counter._delta:"<< counter._delta;
                        ...
                        LOG(3)<<"MoreDbRocksRecoveryUnit::_commit counterKey:"<<delta.first
                          <<",newValue:"<<newValue;
                        pair.first->GetCounterManager()->updateCounter(delta.first, newValue, nullptr);
                    }
                }
                _moreDBDeltaCounters.clear();
...
void MoreDbRocksRecoveryUnit::incrementCounter(const rocksdb::Slice& counterKey,
    ...
    LOG(3)<<"@@## MoreDbRocksRecoveryUnit::incrementCounter"
...
RocksTransaction* MoreDbRocksRecoveryUnit::transaction()
{
    ...
    if (it == _moreTransaction.end()) {
         RocksTransaction* rt = new RocksTransaction(_db->GetTransactionEngine());
         _moreTransaction[_db] = rt;
         return rt;
    } else {
        return it->second;
    }
