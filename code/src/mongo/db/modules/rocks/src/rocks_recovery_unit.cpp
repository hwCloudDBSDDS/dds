...
namespace mongo {
    ...
    RocksRecoveryUnit::~RocksRecoveryUnit() {
        if (_preparedSnapshot) {
            ...
            getDB()->ReleaseSnapshot(_preparedSnapshot);
            _preparedSnapshot = nullptr;
        }
        _abort();
    ...
    Status RocksRecoveryUnit::setReadFromMajorityCommittedSnapshot() {
        if (!GetSnapshotManager()->haveCommittedSnapshot()) {
    ...
    void RocksRecoveryUnit::_commit(OperationContext* txn) {
        bool rrucmtPrpFlg = false;
        bool rrucmtDbWrtFlg = false;
        ...
        if (wb->Count() != 0) {
            ...
            bool isSplitOnGoing = _db->GetTransactionEngine()->getSplitIsOnGoing();
            throwExceptionIfWriteBatchBelongsToChildSide(txn, &_writeBatch, _db, isSplitOnGoing);
            ...
            try{
                ...
                writeOptions.disableWAL = !_db->IsDurable();
    ...
    void RocksRecoveryUnit::decCountOfActive(bool isSplitOnGoing, ChunkRocksDBInstance* db) 
    {
        if (isSplitOnGoing) {
            ...
            db->GetTransactionEngine()->subtractCountOfActiveWriteBatchesOutsideOfSplit();
        }

        if (db->GetTransactionEngine()->getCountOfActiveWriteBatchesDuringSplit() == 0) {
            stdx::unique_lock<stdx::mutex> lock(db->_mutex);
            db->_duringSplitCondVar.notify_all();
        }

        if (db->GetTransactionEngine()->getCountOfActiveWriteBatchesOutsideOfSplit() == 0) {
            stdx::unique_lock<stdx::mutex> lock(db->_mutex);
            db->_outsideOfSplitCondVar.notify_all();
        }
    }
    ...
    void RocksRecoveryUnit::addCountOfActive(bool isSplitOnGoing, ChunkRocksDBInstance* db)
    {
        if (isSplitOnGoing) {
            ...
            db->GetTransactionEngine()->increaseCountOfActiveWriteBatchesOutsideOfSplit();
        }
    }
    ...
    const rocksdb::Snapshot* RocksRecoveryUnit::snapshot() {
        if (_readFromMajorityCommittedSnapshot) {
            if (_snapshotHolder.get() == nullptr) {
                _snapshotHolder = GetSnapshotManager()->getCommittedSnapshot();
            }
        ...
        if (!_snapshot) {
            ...
            transaction()->recordSnapshotId();
            _snapshot = getDB()->GetSnapshot();
        }
        return _snapshot;
    }
    ...
    RocksIterator* RocksRecoveryUnit::NewIteratorNoSnapshot(rocksdb::DB* db, std::string prefix, rocksdb::ColumnFamilyHandle* cf) {
        ...
        rocksdb::ReadOptions options;
        options.iterate_upper_bound = upperBound.get();
        auto iterator = db->NewIterator(rocksdb::ReadOptions(), cf);
