...
namespace mongo {
    ...
    protected:
        ...
        IndexRecordFormatterBase(const IndexDefinition& indexDefinition) {
            ...
            isUniqueIndex = IndexDescriptor::hasUniqueModifier(
                infoObj);  //  We never should have _id index here
        }
    };
    ...
    protected:
        ...
        public:
            virtual const char* Name() const {
                return "ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage::"
                       "ShardKeyFromPrimaryKeyExtractor";
            }
            ...
            virtual bool InRange(const rocksdb::Slice& dst) const { return true; }
        ...
        class CFDescriptionBase : public IChunkColumnFamilyDescription,
                                  public StorageReferenceHolder {
        public:
            CFDescriptionBase(const Storage& storage) : StorageReferenceHolder(storage) {}

            virtual bool DoesRecordBelongToChunk(const rocksdb::Slice& key,
        ...
        public:
            ...
            virtual rocksdb::KeyRangeCheckResult DoesKeyRangeBelongToChunk(
                const rocksdb::Slice& rangeKeyLow,
                const rocksdb::Slice& rangeKeyHigh) const override {
                //  Don't extract exact shard key since key always starts with shard key
                return storage.DoesShardKeyRangeBelongToChunk(storage.Adjust(rangeKeyLow),
                                                              storage.Adjust(rangeKeyHigh));
            }

            virtual rocksdb::Slice GetShardKey(const rocksdb::Slice& key,
                                               const rocksdb::Slice& value) const override {
            ...
            virtual std::shared_ptr<const rocksdb::SliceTransform> GetPrefixExtractor() const {
                return std::shared_ptr<const rocksdb::SliceTransform>(
                    new ShardKeyFromPrimaryKeyExtractor());
            }
        };
        ...
        public:
            ...
            virtual const char* GetName() const override { return "index"; }
        ...
        public:
            IndexRecordFormatter(const Storage& storage, const IndexDefinition& index)
                : StorageReferenceHolder(storage) {
                prefix = encodePrefix(index.GetIndexId());
            ...
            virtual mongo::Ordering GetOrdering() const override { return keyOrdering; }

            virtual bool IsUnique() const override { return isUnique; }

            virtual bool IsLocalKey() const override { return isLocalKey; }
            ...
            virtual rocksdb::Status GenerateRecord(
                ...
                std::pair<std::string, std::string>& recordKeyValuePair) const override {
                SecondaryIndexBuilder keyBuilder;
                SecondaryIndexBuilder valueBuilder;

                keyBuilder.Append(prefix);
                keyBuilder.Append(key, keyOrdering);
                index_LOG(1) << "IndexRecordFormatter::GenerateRecord IndexRecordFormatter "
                                "GenerateRecord unique:"
                             << isUnique;
            ...
            virtual rocksdb::Status GenerateCursorPosition(const mongo::BSONObj& key,  //  Index key
                                                           KeyString::Discriminator discriminator,
                                                           std::string& recordKey) const override {
                SecondaryIndexBuilder keyBuilder;
            ...
            virtual rocksdb::Status ParseRecord(
                ...
                if (key) *key = keyReader.GetSecondaryKey(keyOrdering);

                if (documentRecordId) *documentRecordId = keyReader.GetRecordId();

                return rocksdb::Status::OK();
            }

            virtual const std::string& GetPrefix() const { return prefix; }
        };
        ...
        protected:
            static bool DoesShardKeyContainIdField(const ShardKeyPattern& shardKeyPattern) {
                for (const mongo::BSONElement& element : shardKeyPattern.toBSON()) {
                    if (strcmp(element.fieldName(), "_id") == 0) return true;
                }
                return false;
            }
        ...
        public:
            ...
            RecordId GenerateRecordId(const mongo::BSONObj& document) const {
                ...
                if (shardKey.objsize() > MaxKeySize) {
                    index_err() << "shardKey too long : " << shardKey
                                << "; size: " << shardKey.objsize();
                    return RecordId();
                }
            ...
            BSONObj GetShardKeyBSON(const rocksdb::Slice& key, bool includeFieldNames) const {
                ...
                rocksdb::Slice shardKey = formatter.GetShardKey();
                return KeyString::toBson(shardKey.data(), shardKey.size(), ordering, outTypeBits,
                                         includeFieldNames ? &pattern.getKeyPattern() : nullptr);
            }
        };

        rocksdb::Slice Adjust(const rocksdb::Slice& slice) const { return RemovePrefix(slice); }

        int CompareWithLowKey(const rocksdb::Slice& key) const {
            if (lowKey.empty()) return 1;
            return key.compare(lowKey);
        }

        int CompareWithHighKey(const rocksdb::Slice& key) const {
            if (highKey.empty()) return -1;
        ...
        virtual rocksdb::KeyRangeCheckResult DoesShardKeyRangeBelongToChunk(
            const rocksdb::Slice& rangeKeyLow, const rocksdb::Slice& rangeKeyHigh) const {
    ...
    public:
        ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage(const ChunkMetadata& metadata)
            ...
            shardKeyInfo =
                stdx::make_unique<ShardKeyInfo>(metadata.GetCollection().getKeyPattern());

            SetKeyRange(metadata.GetKeyRange());
        }

        virtual rocksdb::Status Init(ChunkRocksRecordStore& rs) { return rocksdb::Status::OK(); }
        ...
        virtual void SetKeyRange(const ChunkMetadata::KeyRange& range) {
            ...
            PrimaryKeyBuilder::GenerateRangeBorderKey(range, shardKeyInfo->ordering, lowKey,
                                                      highKey, highKeyNoEnd);
        }
        std::string GetLowKey(void) override { return lowKey; }

        std::string GetHighKey(void) override { return highKeyNoEnd; }
        ...
        BSONObj GetShardKeyBSON(mongo::RecordId& documentRecordId) const {
            rocksdb::Slice primaryKey(documentRecordId.repr().Data(),
                                      documentRecordId.repr().Size());
