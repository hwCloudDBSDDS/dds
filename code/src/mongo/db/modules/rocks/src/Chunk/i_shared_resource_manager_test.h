...
namespace mongo {
    ...
    namespace TransactionLog {
        ...
        uint64_t lsn_stub = 0;
        static inline uint64_t GetLsnStub() { return lsn_stub; }
        static inline uint64_t IncLsnStub() {
            lsn_stub++;
            return lsn_stub;
        }
        static inline void ResetLsnStub() { lsn_stub = 0; }

        static inline void CopyBuf(uint8_t* buf1, uint32_t len1, const uint8_t* buf2,
                                   uint32_t len2) {
            uint32_t i = 0;
            for (; ((i < len1) && (i < len2)); i++) {
            ...
            switch (oper_type) {
                case SharedResourceOperationType::AddFilter:
                    ...
                    printf("----[%u] LSN(%lu) LogType(%d) OperType(%s) ResourceId(%lu) \n", index,
                           lsn, log_type, oper_type_name.c_str(),
                           SharedResourceIdToUint64(filter_oper->GetResource()));
                    delete filter_oper;
                    break;
                ...
                case SharedResourceOperationType::AddReference:
                    ...
                    printf("----[%u] LSN(%lu) LogType(%d) OperType(%s) ResourceId(%lu) \n", index,
                           lsn, log_type, oper_type_name.c_str(),
                           SharedResourceIdToUint64(reference_oper->GetResource()));
                ...
                case SharedResourceOperationType::Processed:
                    ...
                    printf("----[%u] LSN(%lu) LogType(%d) OperType(%s) ProcessedLsn(%lu) \n", index,
                           lsn, log_type, oper_type_name.c_str(),
                           processed_oper->GetProcessedLSN());
                ...
                case SharedResourceOperationType::StartSplit:
                    ...
                    printf("----[%u] LSN(%lu) LogType(%d) OperType(%s) SplitID(%lu) \n", index, lsn,
                           log_type, oper_type_name.c_str(), split_oper->GetId());
                    delete split_oper;
                    break;
        ...
        void PrintOperations(SharedResourceOperationLogRecordProvider* log_provider,
                             std::string header) {
            ...
            for (auto resource : resources) {
                ...
                if ((count <= 101) || (0 == (count % 200))) {
                    printf("      [%u] resourceID(%lu)\n", (count - 1),
                           SharedResourceIdToUint64(resource));
                }
            }
        ...
        public:
            ...
            void ReplayLogRecord() {
                if (printf_flag_) {
                    printf("-- begin of ReplayLogRecord \n");
                }
                uint64_t max_lsn = 0;
                ...
                if (printf_flag_) {
                    printf("-- end of ReplayLogRecord. max_lsn(%lu) \n", max_lsn);
                }
            }
            ...
            void Init() {
                for (auto writer : writers_) {
                    ...
                    writer->GetProvider()->InitializationBegin(LogReplayOrder::Forward,
                                                               writer->GetLogTypes());
                }
                ReplayLogRecord();
                for (auto writer : writers_) {
            ...
            const uint32_t GetLogRecordsNumber() const { return log_records_.size(); }
            ...
            bool CheckLogRecordNumber(const uint32_t log_record_num, const uint32_t filter_num,
                ...
                return ((GetLogRecordsNumber() == log_record_num) &&
                        (_log_record_num == log_record_num) && (_filter_num == filter_num) &&
        ...
        public:
            ...
            bool CheckLogRecordNumber(const uint32_t log_record_num, const uint32_t filter_num,
                                      const uint32_t loadbalacnce_num) {
                return log_store_->CheckLogRecordNumber(log_record_num, filter_num,
                                                        loadbalacnce_num);
            }
            ...
            void Step_SplitSuccess(
                std::shared_ptr<SplitDescription> description, uint64_t* register_list,
                uint32_t size, SharedResourceModule_Stub& right_module,
                bool log_end_oper_fail = false);  // If fail to write the log of CommitSplit or not.
            void Step_SplitRollback(std::shared_ptr<SplitDescription> description,
                                    uint64_t* register_list, uint32_t size,
                                    bool log_end_oper_fail = false);  // If fail to write the log of
                                                                      // RollbackSplit or not.
