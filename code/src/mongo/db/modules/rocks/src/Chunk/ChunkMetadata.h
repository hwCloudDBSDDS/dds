...
#define RES_LOG_ERROR(status, operation)   ResultHandling::LogError(status, operation, __FILE__, __LINE__, __FUNCTION__)
...
public:
    ...
    uint64_t GetIndexId() const
    {
        return indexId;
    }

    const mongo::BSONObj& GetDefinition() const
    {
        return definition;
    }
    ...
    bool IsUnique() const
    {
        return IndexDescriptor::hasUniqueModifier(definition) || IsId();
    }
    
    BSONObj GetKeyPattern() const
    {
        return IndexDescriptor::getKeyPattern(definition);
    }

    bool IsId() const
    {
        return IndexDescriptor::isIdIndexPattern(GetKeyPattern());
    }
};
...
enum class IndexedRecordStorageId : uint16_t
{
    ...
    //      Index:
    //          Unique: { indexPrefixId + key } -> { valueSize + recordId + typeBits + shardKeyOffset }
...
#define CHUNK_METADATA_PARSE_ELEMENT(elem, field)           \
        if(!BsonParser::GetValue(elem, field))              \
...
public:
    ...
    void setCollection(const CollectionType &coll)
    {
        collection = coll;
    }
    ...
    IndexedRecordStorageId GetIndexedRecordStorageId() const
    {
        return indexedRecordStorageId;
    }

    const std::vector<IndexDefinition>& GetIndexes() const
    {
        return indexes;
    }
    ...
    public:
        ...
        const mongo::BSONObj& GetKeyLow() const
        {
            return keyLow;
        }
        
        const mongo::BSONObj& GetKeyHigh() const
        {
            return keyHigh;
        }
        ...
        {
            return
                BsonParser::AreEqual(keyLow, keyRange.keyLow) && 
                BsonParser::AreEqual(keyHigh, keyRange.keyHigh);
        }
    };

    KeyRange GetKeyRange() const
    {
        return KeyRange(chunk.getMin(), chunk.getMax());
    }
...
public:
    ...
    ChunkMetadata(IndexedRecordStorageId indexedRecordStorageId, const ChunkType& chunk, const CollectionType& collection) : 
            indexedRecordStorageId(indexedRecordStorageId), chunk(chunk), collection(collection)            
    {
        InitializeIndexFromCollection();
    }

    static rocksdb::Status Parse(const rocksdb::Slice& slice, std::unique_ptr<ChunkMetadata>& metadata)
    {
        mongo::BSONObj bson(slice.data());
        RES_RET(Parse(bson, metadata));
    }

    static rocksdb::Status Parse(const mongo::BSONObj& bson, std::unique_ptr<ChunkMetadata>& metadata)
    {
        auto meta = mongo::stdx::make_unique<ChunkMetadata>();        

        std::array<mongo::StringData,8> fieldNames{"metadataVersion", "indexedRecordStorageId", "chunk", "collection"};
