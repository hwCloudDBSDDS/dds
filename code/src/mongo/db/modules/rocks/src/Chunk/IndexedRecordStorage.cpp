...
rocksdb::Status IIndexedRecordStorage::Create(const ChunkMetadata& metadata, std::unique_ptr<IIndexedRecordStorage>& storage)
{
    mongo::log() << "IIndexedRecordStorage::Create " << metadata.ToString();
    switch(metadata.GetIndexedRecordStorageId())
    {
        ...
        {
            mongo::log() << "IIndexedRecordStorage::Create ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage";
            storage = stdx::make_unique<ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage>(metadata);
            return rocksdb::Status::OK();
        }
