...
std::unique_ptr<TransLogRecordStore> CreateRecordStore(std::string& chunk_id,
            std::string& db_path, rocksdb::Env** plog_env) {
    ...
    //plog_client_options.plog_append_no_sync = true;  //single host test , no need fsync for plog append data  to speedup
    ...
    //std::unique_ptr<TransLogRecordStore> trans_log_Record_store(new TransLogRecordStore(*plog_env, db_path));
...
void AddRemoveDescription(std::vector<SharedResourceRemoveDescription>& rmv_desc_list,
                          ...
                          const int remove_result)
{
    SharedResourceRemoveDescription rmv_desc = {GetSharedResourceId(id), shared_flag, remove_result};
...
TEST(TransLogRecordStoreTest, TransLogRecordStore_BatchWritePosixEnv)
{
    ...
    std::unique_ptr<TransLogRecordStore> trans_log_record_store(new TransLogRecordStore(env, path_tmp));
// 1 Before the split
    ...
    s = filter_manager.OnResourcesDeletion(remove_list);
    ASSERT_TRUE(s.ok());
...
// 2 Start split.
    ...
    remove_list.clear();
    AddRemoveDescription(remove_list, 21, false, RET_ERR);
    AddRemoveDescription(remove_list, 20, false, RET_ERR);
    AddRemoveDescription(remove_list, 15, false, RET_ERR);
    s = filter_manager.OnResourcesDeletion(remove_list);
    ASSERT_TRUE(!s.ok());
    ASSERT_TRUE(CheckRemoveResult(remove_list, 21, false, RET_ERR));
...
//3 Write shared resource reference logs to right side.
    ...
    std::unique_ptr<TransLogRecordStore> right_trans_log_record_store(new TransLogRecordStore(env, path_tmp2));
...
// 5 After split.
    ...
    remove_list.clear();
    AddRemoveDescription(remove_list, 100, false, RET_ERR);
    AddRemoveDescription(remove_list, 86, false, RET_ERR);
    s = filter_manager.OnResourcesDeletion(remove_list);
    ASSERT_TRUE(s.ok());
    ASSERT_TRUE(CheckRemoveResult(remove_list, 100, true, RET_OK));
    ASSERT_TRUE(CheckRemoveResult(remove_list, 86, true, RET_OK));
...
// 6.2 replay left
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test_left(new TransLogRecordStore(env, path_tmp));
    // init
...
// 6.2 replay right
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test_right(new TransLogRecordStore(env, path_tmp2));
    // init
...
// 7.1 Write shared resource reference logs to right side.
    ...
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test(new TransLogRecordStore(env, path_tmp3));
...
// 7.2 replay right
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_1(new TransLogRecordStore(env, path_tmp3));
    // init
...
// 5 After split.
    ...
    remove_list.clear();
    AddRemoveDescription(remove_list, 1, false, RET_ERR);
    s = filter_manager.OnResourcesDeletion(remove_list);
    ASSERT_TRUE(s.ok());
    ASSERT_TRUE(CheckRemoveResult(remove_list, 1, true, RET_OK));
...
TEST(TransLogRecordStoreTest, TransLogRecordStore_BatchWrite_PlogEnv)
{
    ...
    std::unique_ptr<TransLogRecordStore> trans_log_record_store = CreateRecordStore(chunk_path, db_path, &env);
// 1 Before the split
    SharedResourceOperationLogRecordProvider sro_provider;
    SharedResourceManager filter_manager;
    filter_manager.SetLogRecordProvider(&sro_provider);
...
//3 Write shared resource reference logs to right side.
    ...
    SharedResourceOperationLogRecordProvider right_sro_provider;
    SharedResourceManager right_filter_manager;
    std::unique_ptr<TransLogRecordStore> right_trans_log_record_store = CreateRecordStore(chunk_path2, db_path, &env);
    right_filter_manager.SetLogRecordProvider(&right_sro_provider);
    right_sro_provider.SetSharedResourceManager(&right_filter_manager);
    right_trans_log_record_store->RegisterProvider(right_sro_provider);
    right_trans_log_record_store->RegisterProvider(right_filter_manager);
...
// 6.2 replay left
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test_left = CreateRecordStore(chunk_path, db_path, &env);
    // init
    SharedResourceOperationLogRecordProvider sro_provider_test_left;
    SharedResourceManager filter_manager_test_left;
    filter_manager_test_left.SetLogRecordProvider(&sro_provider_test_left);
...
// 6.2 replay right
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test_right = CreateRecordStore(chunk_path2, db_path, &env);
    // init
    SharedResourceOperationLogRecordProvider sro_provider_test_right;
    SharedResourceManager filter_manager_test_right;
    filter_manager_test_right.SetLogRecordProvider(&sro_provider_test_right);
...
// 7.1 Write shared resource reference logs to right side.
    SharedResourceOperationLogRecordProvider provider_test;
    SharedResourceManager filter_manager_test;
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_test = CreateRecordStore(chunk_path3, db_path, &env);
    filter_manager_test.SetLogRecordProvider(&provider_test);
    provider_test.SetSharedResourceManager(&filter_manager_test);
    trans_log_record_store_test->RegisterProvider(provider_test);
    trans_log_record_store_test->RegisterProvider(filter_manager_test);
...
// 7.2 replay right
    std::unique_ptr<TransLogRecordStore> trans_log_record_store_1 = CreateRecordStore(chunk_path3, db_path, &env);
    // init
    SharedResourceOperationLogRecordProvider sro_provider_1;
    SharedResourceManager filter_manager_1;
    filter_manager_1.SetLogRecordProvider(&sro_provider_1);
