...
#define INVALID_LSN (std::numeric_limits<uint64_t>::max())
        ...
        public:
            LogRecord(LogRecordType type, uint64_t sn, const rocksdb::Slice& slice)
                : type(type), lsn(sn), slice(slice) {
                invariant(slice.data() != nullptr);
                invariant(slice.size() != 0);
                invariant(lsn != INVALID_LSN);
            }
            ...
            LogRecord(LogRecordType type, uint64_t sn, const char* data, size_t size)
                : LogRecord(type, sn, rocksdb::Slice(data, size)) {}
            LogRecord(LogRecordType type, const char* data, size_t size)
                : LogRecord(type, rocksdb::Slice(data, size)) {}

            const LogRecordType GetType() const { return type; }

            const rocksdb::Slice& GetSlice() const { return slice; }

            const char* GetData() const { return slice.data(); }

            const size_t GetSize() const { return slice.size(); }

            void SetLsn(uint64_t s) { lsn = s; }

            const uint64_t GetLsn() const { return lsn; }
            ...
            // Only the owner of the data can call this method.
            void ClearData() {
                if (!slice.empty()) {
                    delete GetData();
                    slice.clear();
        ...
        //  The instance of this interface will be provided to Log Record Provider during
        //  initialization.
        //
        class ILogRecordWriter {
        public:
            // write mutilpue records in one write
            virtual rocksdb::Status WriteRecord(std::vector<LogRecord>& records,
                                                bool allow_check_point) = 0;

            // write one record in one write
            virtual rocksdb::Status WriteRecord(LogRecord& record, bool allow_check_point) = 0;
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        //
        //  Interface represent particular record provider, which needs to save records in log
        //  structured way.
        //
        ...
        //      1.2. ILogRecordStore provides the order in which it's gonig to replay logs. If
        //      provider doesn't
        //          support this order it can return NotSupported. In this case ILogRecordProvider
        //          can try
        //          to propose different order.
        //      1.3. ILogRecordStore associate provider with the list of record types, that this
        //      provider
        //          supports. It get them from supportedRecordTypes argument of InitializationBegin.
        //      2. After opening of record source (e.g. Chunk loading) ILogRecordStore calls
        //      ReplayLogRecord for
        //          each record it has saved after the start of latest successfull checkpoint. Note:
        //          records
        //          can be replayed in arbitrary order. If order matters for provider it needs to
        //          maintain LSN and
        //          resort records. During the replay, provider can let ILogRecordStore know that it
        //          doesn't need
        //          more records.
        //      2.1. When all records are replayed, ILogRecordStore is ready to accept write
        //      requests from provider
        //          and call InitializationEnd provider "writer" object. Providers need to save this
        //          objects to write
        //          records and checkpoint.
        //      3. When ILogRecordStore observes that there are too many log records, it tries to
        //      reduce amount of
        //          records by writing checkpoint. For that it firstly calls CheckpointBegin for
        //          each provider.
        //      3.1. Then ILogRecordStore calls WriteCheckpoint for each provider.
        //      3.2. When all providers have written their checkpoints ILogRecordStore calls
        //      CheckpointEnd.
        //      4. When log source is closing (e.g. chunk offload) ILogRecordStore calls
        //      UnloadProvider. It's
        ...
        public:
            //
            //  Called for each provider before log replay. Provider needs to return list of
            //  supported record types
            //  in 'supportedRecordTypes'. If provider doesn't support replay order it can return
            //  NotSupported.
            //
            virtual rocksdb::Status InitializationBegin(
                LogReplayOrder replayOrder, std::vector<LogRecordType>& supportedRecordTypes) = 0;

            //
            //  Called by ILogRecordStore for each record related to current provider and saved in
            //  store after the
            //  start of latest successfull checkpoint. This function will be called only between
            //  InitializationBegin and
            //  InitializationEnd calls. If provider has enough data it can set needMoreRecords to
            //  false and store won't
            //  call provider after that.
            //
            virtual rocksdb::Status ReplayLogRecord(const LogRecord& record,
                                                    bool& needMoreRecords) = 0;
            ...
            //
            virtual rocksdb::Status WriteCheckpoint(bool realloc_lsn = false) = 0;
