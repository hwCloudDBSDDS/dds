...
public:
    ...
    const rocksdb::Slice& GetKey() const
    {
        return key;
    }
};
...
public:
    ...
    RecordIdFormatter()
    {
        static_assert(sizeof(TailByte) == 1, "TailByte must be 1 byte");
    }    

    static void EncodeHeader(StackBufBuilder& buffer, size_t shardKeySize, const KeyString::TypeBits& typeBits)
    {
    ...
    {
        //  key = ShardKey + _id
        //  buffer = ShardKey + _id + typeBits (written with readFromEnd) + tail (1 or 2 bytes wit TailByte last)
        if(buffer.size() < 3)
            RES_RET(rocksdb::Status::Corruption());
        ...
        {
            error()<<"RecordIdFormatter::DecodeHeader version must be 0.";
        ...
        if(buffer.size() < tailSize + 2)
            RES_RET(rocksdb::Status::Corruption());

        int typeBitsSize = KeyString::TypeBits::getRemainingBytesSize(buffer[buffer.size() - tailSize - 1])+1;    
        ...
        if(key[keySize-1] != KeyString::kEnd)
            RES_RET(rocksdb::Status::Corruption());

        return rocksdb::Status::OK();
    }

    const rocksdb::Slice& GetShardKey() const
    {
        return shardKey;
    }
};
...
protected:
    ...
    union TailByte
    {
        struct
        {
            //  Offset of the primary key from the begining of the record: key or value, depending of the primaryKeyInValue
            uint8_t primaryKeyOffset:bitsInTailOffset; 

            //  If set to true, byte below current (tail) contains remaining high bits of the primaryKeyOffset
            bool continuation:1;
...
public:
    SecondaryKeyFormatter()
    {
        static_assert(sizeof(TailByte) == 1, "TailByte must be 1 byte");
    }    

    static void EncodeHeader(StackBufBuilder& buffer, const KeyString::TypeBits& typeBits, bool primaryKeyInValue, uint16_t primaryKeyOffset)
    {
        ...
        if(primaryKeyInValue)
            primaryKeyOffset = buffer.len();    //  Offset from the end
    ...
    rocksdb::Status DecodeHeader(const rocksdb::Slice& indexKey, const rocksdb::Slice& indexValue, size_t prefixSize = 0)
    {
        //  buffer = [PrimaryKey]+TypeBits (written with readFromEnd) + tail (1 or 2 bytes with TailByte last)
        ...
        {
            error()<<"SecondaryKeyFormatter::DecodeHeader version must be 1."
                   <<", indexKey:"<<indexKey.ToString() <<", indexKey size:"<<indexKey.size()
                   <<", indexValue:"<<indexValue.ToString()<< ", indexValue size:"<< indexValue.size();
            RES_RET(rocksdb::Status::NotSupported());
        }
        ...
        if(tailByte.continuation)
        {
            if(indexValue.size() < 2)
                RES_RET(rocksdb::Status::Corruption());
            primaryKeyOffset |= ((uint16_t)(indexValue[indexValue.size() - 2]) << bitsInTailOffset);
        }

        size_t tailSize = tailByte.continuation ? 2 : 1;
        if(indexValue.size() < tailSize + 1)
            RES_RET(rocksdb::Status::Corruption());

        int typeBitsSize = KeyString::TypeBits::getRemainingBytesSize(indexValue[indexValue.size() - tailSize - 1])+1;    
        if(tailSize+typeBitsSize > indexValue.size())
            RES_RET(rocksdb::Status::Corruption());
        ...
        if(tailByte.primaryKeyInValue)
        {   //  Offset from the tail
            if(primaryKeyOffset <= typeBitsSize || tailSize + primaryKeyOffset > indexValue.size())
                RES_RET(rocksdb::Status::Corruption());

            key = indexKey;
            primaryKey = rocksdb::Slice(valueEnd - tailSize - primaryKeyOffset, primaryKeyOffset - typeBitsSize);
        }
        else
        {   //  Offset from the begining
            if(primaryKeyOffset >= indexKey.size() || primaryKeyOffset == 0)
            {
                ...
                errorMessage    << " primaryKeyOffset=" << primaryKeyOffset
                                << " tailSize=" << tailSize
                                << " typeBitsSize=" << typeBitsSize;
                RES_RET(rocksdb::Status::Corruption(errorMessage.str()));
            }

            key = rocksdb::Slice(indexKey.data(), primaryKeyOffset);
            primaryKey = rocksdb::Slice(indexKey.data() + primaryKeyOffset, indexKey.size() - primaryKeyOffset);
        }
    ...
    rocksdb::Status Parse(const std::pair<rocksdb::Slice, rocksdb::Slice>& indexKeyValuePair, size_t prefixSize)
    {
    ...
    {
        return RecordId(primaryKey.data(), primaryKey.size());
    }
...
public:
    ...
    {
        return std::string(getBuffer(), getSize());
    }
};
...
class PrimaryKeyBuilder : public ChunkKeyString
{
    ...
    {
        return RecordId(getBuffer(), getSize());
    }
...
public:
    ...
    static void GenerateRangeBorderKey(
        const ChunkMetadata::KeyRange& range,
        Ordering ordering,
        std::string& lowKey,
        std::string& highKey,
...
public:
    ...
    void Append(const BSONObj& obj, const Ordering ord, Discriminator discriminator = Discriminator::kInclusive)
    {
    ...
    const TypeBits& get_TypeBits()const{
        return getTypeBits();
    }
    ...
    {
        invariant(
            (primaryKeyOffset == -1 || keyBuilder.primaryKeyOffset == -1) &&
        ...
        SecondaryKeyFormatter::EncodeHeader(_buffer, keyBuilder._typeBits, primaryKeyInValue, (uint16_t)primaryKeyOffset);
    }
};
