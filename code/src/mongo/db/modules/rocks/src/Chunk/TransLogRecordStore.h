...
struct CheckpointDiskFormat{
    uint64_t  checkpoint_start_lsn; // if checkpoint start, lsn is setted to lsn of checkpoint start
...
class TransLogFileWriter {
 ...
 public:
  TransLogFileWriter(std::unique_ptr<rocksdb::WritableFile>&& file,
                           uint64_t  file_sn)
...
class TranslogMutexLock {
 public:
  explicit TranslogMutexLock(TranslogMutex* mutex) : mutex_(mutex) {
    mutex_->Lock();
  }
...
struct CheckpointLocation{
    ...
    uint64_t  self_lsn;
    // in the case of invalid checkpoint and commit, checkpoint_start_offset is offset of checkpoint start
...
public:
    ...
    rocksdb::Status RewriteImportantFailLog(void);

    // write trans log for checkpoint
    rocksdb::Status LogCheckPointState(const SystemTlogOperationType type, const uint64_t checkpoint_start_lsn, uint64_t& new_log_lsn);
    ...
    bool DecodeCheckpoint(const uint64_t file_sn,
        const uint64_t offset_in_file,
        const TransLogSubTlv* sub_tlv,
        bool& get_checkpoint);

    bool InvertGetCheckpoint(const uint64_t file_sn,
        const uint64_t offset_in_file,
        const rocksdb::Slice& data,
        bool& get_checkpoint,
        uint32_t& decode_len);

    rocksdb::Status InvertReadTransLogFils(const uint64_t file_sn,
            char *buf, const uint32_t buf_len, bool& get_checkpoint);

    rocksdb::Status GetCheckpoint4Replay(bool& need_replay);

    bool GetTransLogReplayStartAndEndPos(std::vector<ValidStartEndPosition>* valid_position_list);
    // Forward replay trans log
    rocksdb::Status ForwardReplayTransLog(std::vector<RecordTypeProvider>& record_type_provider,
        uint64_t&  max_replayed_lsn);

    rocksdb::Status ForwardReadOneTransLogFile(std::vector<RecordTypeProvider>& record_type_provider,
        const uint64_t file_sn, uint32_t start_offset,
        const uint32_t end_offset, const uint64_t file_size,
    ...
    rocksdb::Status CreateNewDir(const std::string& fileName);
    //
    //  Destructor
...
private:
    ...
    const uint32_t         pad_size_ = 128;

    // data in Append() is no longer than 1MB
    const uint32_t         max_buf_size_ = 1048576;
...
private:
    std::string MakeFileName(uint64_t file_sn) {
       char buf[64];
       snprintf(buf, sizeof(buf), "/%llu.tlog",
    ...
    ILogRecordProvider* GetRecordProvider(const LogRecordType type,
         std::vector<RecordTypeProvider>& record_type_provider_list,
         bool& isNeedMoreRecords);
    ...
    rocksdb::Status DecodeOneHeadTlv(
