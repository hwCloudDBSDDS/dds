...
{
public:
    ILogRecordProviderTest(std::string& provider_name){
    ...
    //
    rocksdb::Status InitializationBegin(LogReplayOrder replayOrder, std::vector<LogRecordType>& supportedRecordTypes)
    {
    ...
    void SetNeedMoreRecordState(bool needMoreRecord)
    {
        needMoreRecords_ = needMoreRecord;
    }
    ...
    rocksdb::Status WriteCheckpoint() {
       return log_writer_->WriteRecord(written_record_);}
    ...
    rocksdb::Status RewriteImportantFailedLog(void) { return rocksdb::Status::OK();}

    rocksdb::Status WriteRecord(std::vector<LogRecord>& record){
        rocksdb::Status s = log_writer_->WriteRecord(record);
        if(s.ok()){
            char *buf = nullptr;
...
std::unique_ptr<TransLogRecordStore> CreateRecordStore(std::string& chunk_id,
    ...
    //std::unique_ptr<TransLogRecordStore> trans_log_Record_store(new TransLogRecordStore(*plog_env, db_path_tmp));
...
TEST(TransLogRecordStoreTest, checkpointtest_PosixEnv)
{
    ...
    char buf[1024] = "this is LogRecordType::ShareResourceLogthis is LogRecordType::ShareResourceLog"
        "this is LogRecordType::ShareResourceLogthis is LogRecordType::ShareResourceLogthis is LogRecordType::ShareResourceLog"
