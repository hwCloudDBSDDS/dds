...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
//  In this section we define some functions from libraries which we don't want to link for some
//  reasons.
//  Dump approach but it saves time. Need to fix it someday.
//

#include "index-base/index_base.h"
...
void* IndexThreadLocal::GetKey(eThreadLocalKey eKey, int* pvalue_size) { return nullptr; }
...
namespace mongo {
    ...
    bool RocksEngine::initRsOplogBackgroundThread(StringData ns) { return false; }
    ...
    void assembleResponse(OperationContext* txn, Message& m, DbResponse& dbresponse,
    ...
    TEST(RocksRecordChunkTest, Metadata)
    {
        std::string assignmentCommand =
            ...
            "collection: "  //  12:58
            "{" //  1:59
            "    _id: 'mydb.coll1', lastmodEpoch: ObjectId('594cb211ca101a9e55e067b0'), lastmod: new
    Date(1), dropped: false, prefix: 3, key: { a: 1 }, unique: false," //  153:212
            "    index: [ { v: 2, name: '_id_', ns: 'mydb.coll1', key: { _id: 1 }, prefix: 2 }, {
    ns: 'mydb.coll1', v: 2, key: { uid: 1.0 }, name: 'uid_1', prefix: 3 } ]"  //  156:368
            "}, "   //  3:371
            "chunk: "   //  7:378
            "{" //  1:379
            "    _id: 'mydb.coll1-uid_\"A\"', ns: 'mydb.coll1', min: { uid: 'A' }, max: { uid: 'B'
    }, shard: 'shard00000000', lastmod: 0,"
            "    lastmodEpoch: ObjectId('594b996b77d7de05cce31839'), state: 1, rootFolder:
    'plogcnt1,18446744071691134375-0,' }"
            "}"
            "}";
        ...
        ASSERT_TRUE(metadata1->GetIndexedRecordStorageId() ==
    IndexedRecordStorageId::ShardKeyPrefixedDataWithStandardIndexColumnFamily);
        ...
        ASSERT_TRUE(SimpleBSONObjComparator::kInstance.evaluate(index->GetKeyPattern() ==
    fromjson("{ uid: 1.0 }")));
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////
    */
    ...
    TEST(GlobalConfigTestClass, GlobalConfigMainTest) { GlobalConfigTest::MainTest(); }
    ///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    class ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorageTest {
        static constexpr const char* const ChunkMetadataText =
            ...
            "   status: 1,"
            "	rootFolder: "
            "'plogcnt4,4758145404648494219-0,18446744073463842483-0,16464705921138542587-0,"
            "5122289316238896394-0',"
            ...
            "		{ ns: 'TestDB.TestCol', v: 2, key: { bucket: 1.0, object: 1.0, _id: 1.0 }, "
            "name: 'bucket_1_object_1__id_1', prefix: 3 }"
            "	]"
            "}"
            "}";
        ...
        static constexpr const char* const ChunkMetadataText1 =
            ...
            "chunk:"
            "{"
            "   _id: \"test1.splitTest2-_id_MinKey\", ns: \"test1.splitTest2\", min: { _id: "
            "'AMinKey'  }, max: { _id: 'BMaxKey' }, "
            "   shard: \"shard01\", lastmod: new Date(1), lastmodEpoch: "
            "ObjectId('59c22ec6014763d2c9efdc28'), state: 0,"
            "   rootFolder: "
            "\"plogcnt4,18446744072050574944-0,18446744072831205376-0,4985351734516790617-0,"
            "18446744072530172253-0\","
            "   status: 1,"
            "   processIdentity: 'test'"
            "},"
            "collection: { _id: \"test1.splitTest2$test1.splitTest2-_id_MinKey\","
            "   tableType: 2,"
            "lastmodEpoch: ObjectId('59c22ec6014763d2c9efdc28'), lastmod: new Date(1), dropped: "
            "false, prefix: 1, key: { _id: 1.0 }, unique: false,"
            "indexes: [ { v: 2, name: \"_id_\", ns: \"test1.splitTest2\", key: { _id: 1 }, prefix: "
            "2 } ] }"
            "}";
    ...
    public:
        ...
        */
        ...
        static void CheckUniqueIndexFormatter(
            const std::unique_ptr<IIndexRecordFormatter>& formatter, const RecordId& recordId,
            const char* keyJson) {
            ...
            SecondaryKeyFormatter reader;
            invariantRocksOK(
                reader.DecodeHeader(recordKeyValuePair.first, recordKeyValuePair.second));
        ...
        static void CrashTest(const std::unique_ptr<IIndexRecordFormatter>& formatter) {
            uint8_t indexValue[] = {18,  0,  100, 89,  191, 44, 191, 248, 157, 197,
                                    244, 90, 186, 255, 126, 4,  0,   0,   0,   81};
            uint8_t indexKey[] = {0,   0,   0,   1,   100, 89,  191, 44,  191,
                                  248, 157, 197, 244, 90,  186, 255, 126, 4};
        ...
        static void TestShardKey(ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage& storage,
                                 const char* shardKeys, const char* otherKeys) {
            ...
            std::cout << "TestShardKey expected=" << shardKeyBSONExpect.toString()
                      << " actual=" << shardKeyBSON.toString() << std::endl;
            ASSERT_TRUE(
                SimpleBSONObjComparator::kInstance.evaluate(shardKeyBSON == shardKeyBSONExpect));
        }
        ...
        static void MainTest() {
            ...
            static const char* documentText =
                "{ bucket: 'TestBucket', object: 'TestObject', _id: 'TestId' }";
            ...
            //  Test unique index
            {
                ...
                CheckUniqueIndexFormatter(
                    indexFormatter, recordId,
                    "{ _id: "
                    "'TestIdTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT"
            ...
            //  Test standard index
            {
                ...
                ShardKeyPrefixedDataWithStandardIndexColumnFamilyStorage::
                    ShardKeyFromPrimaryKeyExtractor extractor;
                ...
                // BSONObj shardKeyBSONExpect = stripFieldNames(fromjson("{ bucket: 'TestBucket',
                // object: 'TestObject' }"));
                BSONObj shardKeyBSONExpect =
                    fromjson("{ bucket: 'TestBucket', object: 'TestObject' }");
                ASSERT_TRUE(SimpleBSONObjComparator::kInstance.evaluate(shardKeyBSON ==
                                                                        shardKeyBSONExpect));
            }

            //  Shard key test
            TestShardKey(storage, "bucket: 'TestBucket', object: 1.1", "_id: 'TestId'");
            TestShardKey(storage,
                         "bucket: 999999999999999999, object: ObjectId('59bf2cbff89dc5f45abaff7e')",
                         "_id: 'TestId'");
        }
        ...
        static void TestId() {
            ...
            static const char* documentText =
                "{ 'uid':ObjectId('59bf2cbff89dc5f45abaff7e'), "
                "'_id':ObjectId('59bf2cbff89dc5f45abaff7a') }";
            obj = fromjson(documentText);
