...
namespace mongo {
    ...
    public:
        ChunkRocksIndexBase(mongo::ChunkRocksDBInstance& db,
        ...
        Status insert(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                      bool dupsAllowed) override;
        void unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                     bool dupsAllowed) override;
        std::unique_ptr<SortedDataInterface::Cursor> newCursor(OperationContext* txn,
                                                               bool forward) const override;

    protected:
        //  onInsert allows to check and update inserted KV-pair. It will set empty key if insert is
        //  not required.
        virtual Status onInsert(OperationContext* txn, bool dupsAllowed,
                                std::pair<std::string, std::string>& recordKeyValuePair) = 0;
        virtual Status finalizeUnindex(
            OperationContext* txn, const std::pair<std::string, std::string>& recordKeyValuePair,
            bool dupsAllowed) = 0;

        Status GenerateRecord(const BSONObj& key, const RecordId& loc,
                              std::pair<std::string, std::string>& recordKeyValuePair);
        Status PrepareWrite(OperationContext* txn, const BSONObj& key, const RecordId& loc,
                            std::pair<std::string, std::string>& recordKeyValuePair);
    ...
    public:
        ChunkRocksUniqueIndex(ChunkRocksDBInstance& db,
                              std::unique_ptr<IIndexRecordFormatter> recordFormatter);
    ...
    protected:
        Status onInsert(OperationContext* txn, bool dupsAllowed,
                        std::pair<std::string, std::string>& recordKeyValuePair) override;
        Status finalizeUnindex(OperationContext* txn,
                               const std::pair<std::string, std::string>& recordKeyValuePair,
                               bool dupsAllowed) override;
    };

    class ChunkRocksStandardIndex : public ChunkRocksIndexBase {
    public:
        ChunkRocksStandardIndex(mongo::ChunkRocksDBInstance& db,
                                std::unique_ptr<IIndexRecordFormatter> recordFormatter);
