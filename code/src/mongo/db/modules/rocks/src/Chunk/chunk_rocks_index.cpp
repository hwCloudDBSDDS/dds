...
protected:
    ...
    static uint16_t GetValueSize(const char* block)
    {
    ...
    static const char* GetValue(const char* block)
    {
...
public:
    ...
    static Status CheckValue(const rocksdb::Slice& value)
    {
        if(value.size() >= std::numeric_limits<uint16_t>::max())
            MDB_RET(Status(ErrorCodes::KeyTooLong, "Value generated by IndexRecordFormatter is too long"));
    ...
    {
        return Iterator(slice.data(), slice.data()+slice.size());
    }
    ...
    bool HasElement(const rocksdb::Slice& element) const
    {
    ...
    std::string RemoveElement(Iterator it) const
    {
        ...
        if(partToRemoveSize == slice.size())
            return std::string();
    ...
    static Status CreateList(const std::vector<std::string>& elements, std::string& listToCreate)
    {
        ...
        for(const std::string& element : elements)
            Append(element, listToCreate);

        return Status::OK();
    }

    static Status CreateList(const rocksdb::Slice& singleElement, std::string& listToCreate)
    {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
//  ChunkRocksCursor is heavily based on RocksCursorBase. The difference is that we rely on IIndexRecordFormatter
//  to format keys for us. It allows us to have single class for both unique and non-unique (standard) indexes.
//
//  The logic is: have an iterator which chains current write/update batch from recovery unit and iterator from
...
public:
    ...
    void setEndPosition(const BSONObj& key, bool inclusive) override
    {
        ...
        const auto discriminator = _forward == inclusive ? KeyString::kExclusiveAfter : KeyString::kExclusiveBefore;
        GetFormatter().GenerateCursorPosition(stripFieldNames(key), discriminator, _endPosition);
    }

    boost::optional<IndexKeyEntry> seek(const BSONObj& key, bool inclusive, RequestedInfo parts) override
    {   
        const BSONObj keyWithoutFieldNames = stripFieldNames(key);
        const auto discriminator = _forward == inclusive ? KeyString::kExclusiveBefore : KeyString::kExclusiveAfter;

        seekQuery(keyWithoutFieldNames, discriminator);
        return curr(parts);
    }

    boost::optional<IndexKeyEntry> seek(const IndexSeekPoint& seekPoint, RequestedInfo parts) override
    {
        ...
        const auto discriminator = _forward ? KeyString::kExclusiveBefore : KeyString::kExclusiveAfter;
    ...
    boost::optional<IndexKeyEntry> seekExact(const BSONObj& key, RequestedInfo parts) override
    {
        ...
        index_LOG(2) <<"ChunkRocksCursor::seekExact-0 key: "<< key  <<", prefixedKey:" << prefixedKey
               <<", prefixedKey size:"<<prefixedKey.size();
        rocksdb::Status status = RocksRecoveryUnit::getRocksRecoveryUnit(_txn)
            ->Get(prefixedKey, &_value, _indexer.GetColumnFamily());
        
        index_LOG(2) <<"ChunkRocksCursor::seekExact-1 _value: "<< _value
               <<", _valueSize:" << _value.size();
    ...
    void restore() override
    {
        ...
        if (_iterator && _currentSequenceNumber == ru->snapshot()->GetSequenceNumber())
            return;
...
protected:
    ...
    boost::optional<IndexKeyEntry> curr(RequestedInfo parts) 
    {
        ...
        if (!_indexer.getDBInstance()->isSystemChunk())
        {
            if (/*_indexer.getDBInstance()->bockingReadIOofRight() && */
                ...
                ->DoesRecordBelongToChunk(keySlice(), valueSlice())) 
            {
                LOG(3) << "ChunkRocksCursor::curr()-> belong right db ns: " << _txn->getNs();
            ...
            LOG(3) << "ChunkRocksCursor::curr()-> not belong right db ns: " << _txn->getNs();
        }

        BSONObj keyBSON;
        RecordId recordId;

        index_LOG(1) << "ChunkRocksCursor::curr()-> keySlice: " << keySlice().ToString() << "; key size: "
            << keySlice().size() << "; value: " << valueSlice().ToString() << "; value: size: " << valueSlice().size();

        invariantRocksOK(GetFormatter().ParseRecord(
            std::make_pair(keySlice(), valueSlice()),
            (parts & kWantKey) ? &keyBSON : nullptr,
            &recordId
            ));

        LOG(2) << "ChunkRocksCursor::curr()-> key: " << keyBSON << "; recordid: " << recordId.ToString();
    ...
    void updatePosition()
    {
        ...
        if (!_iterator){
            // _iterator may be null, because for unique index it's possible that we just run seekExact
            // and get value direcly from recovery unit without using iterator. This is how RocksUniqueIndex
            // is working. However currently we don't support it and leave the code just in case.
            //_key = _query;
            
            LOG(2) <<"ChunkRocksCursor::updatePosition-0 _key: "<< _key
                   <<", _key size:"<<_key.size()<<",_query:"<< _query 
        ...
        else
        {
            ...
            LOG(2) <<"ChunkRocksCursor::updatePosition-1 _key: "<< _key
                   <<", _key size:"<<_key.size();
        }
    ...
    {
        _iterator.reset(
            RocksRecoveryUnit::getRocksRecoveryUnit(_txn)->NewIterator(GetPrefix(), false, _indexer.GetColumnFamily())
            );
    }
    ...
    rocksdb::Slice valueSlice() const
    {
        return _iterator ? _iterator->value() : _value;
    }
    
    rocksdb::Slice keySlice() const
    {
        return rocksdb::Slice(_key);
    }

    bool isUniqueIndex() const
    {
        return GetFormatter().IsUnique();
    }
    ...
    //
    //  TODO: currently we use PrefixStrippingIterator, which is default iterator for RocksRecoveryUnit
    //  We don't need prefix stripping since we handle prefixes by ourselves. We need to replace this
    ...
    {
        //  Ugly hack here (will remove later), is based on fact that PrefixStrippingIterator strips prefix
...
public:
    ChunkRocksUniqueBulkBuilder(ChunkRocksUniqueIndex& indexer, OperationContext* txn, bool dupsAllowed) :
    ...
    Status addKey(const BSONObj& newKey, const RecordId& loc)
    {
        ...
        MDB_RIF(_indexer.GetIndexRecordFormatter().GenerateRecord(newKey, loc, recordKeyValuePair));
    ...
    {
        WriteUnitOfWork uow(_txn);
        if (!_valuesForTheSameKey.empty())
            doInsert(); //  Insert the last unique key
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// RocksIndexBase
ChunkRocksIndexBase::ChunkRocksIndexBase(mongo::ChunkRocksDBInstance& db, std::unique_ptr<IIndexRecordFormatter> recordFormatter)
    : RocksIndexBase(&db), _recordFormatter(std::move(recordFormatter))
{
    uint32_t prefix = 0;
    extractPrefix(rocksdb::Slice(_recordFormatter->GetPrefix()), &prefix);
    index_LOG(2) << "ChunkRocksIndexBase::ChunkRocksIndexBase()-> prefix: " << _recordFormatter->GetPrefix()
        << "prefix: " << prefix;
...
Status ChunkRocksIndexBase::GenerateRecord(const BSONObj& key, const RecordId& loc, std::pair<std::string, std::string>& recordKeyValuePair)
{
    MDB_RET(_recordFormatter->GenerateRecord(key, loc, recordKeyValuePair));
}

Status ChunkRocksIndexBase::PrepareWrite(OperationContext* txn, const BSONObj& key, const RecordId& loc, std::pair<std::string, std::string>& recordKeyValuePair)
{
...
Status ChunkRocksIndexBase::insert(OperationContext* txn, const BSONObj& key, const RecordId& loc, bool dupsAllowed)
{
    std::pair<std::string, std::string> recordKeyValuePair;
    MDB_RIF(PrepareWrite(txn, key, loc, recordKeyValuePair));    
    bool belong = getDBInstance()->getColumnFamilyDescriptionInPreperSplit("index")->DoesRecordBelongToChunk(
        rocksdb::Slice(recordKeyValuePair.first), rocksdb::Slice(recordKeyValuePair.second));

    LOG(3) << "ChunkRocksIndexBase::insert() key: ( " << key << " ); loc: ( " << loc.ToString() << "); belong: "
        << belong;
    LOG(3) << "ChunkRocksIndexBase::insert() rocks key: ( " << recordKeyValuePair.first << " ); rocksd value: ( " 
    ...
    if(recordKeyValuePair.first.empty() || recordKeyValuePair.second.empty())   //  Don't need to add anything
        return Status::OK();

    _indexStorageSize.fetch_add(recordKeyValuePair.first.size(), std::memory_order_relaxed);

    _db->GetRocksRecoveryUnit(txn)->writeBatch()->Put(GetColumnFamily(), recordKeyValuePair.first, recordKeyValuePair.second);

    return Status::OK();
}

void ChunkRocksIndexBase::unindex(OperationContext* txn, const BSONObj& key, const RecordId& loc, bool dupsAllowed)
{
...
std::unique_ptr<SortedDataInterface::Cursor> ChunkRocksIndexBase::newCursor(OperationContext* txn, bool forward) const
{
    _db->GetRocksRecoveryUnit(txn);
    return std::unique_ptr<SortedDataInterface::Cursor>(new ChunkRocksCursor(txn, *this, forward));
}
...
//
ChunkRocksUniqueIndex::ChunkRocksUniqueIndex(ChunkRocksDBInstance& db, std::unique_ptr<IIndexRecordFormatter> recordFormatter)
    : ChunkRocksIndexBase(db, std::move(recordFormatter)) {}

Status ChunkRocksUniqueIndex::onInsert(OperationContext* txn, bool dupsAllowed, std::pair<std::string, std::string>& recordKeyValuePair)
{
    auto ru = _db->GetRocksRecoveryUnit(txn);
    ...
    //  Unfortunatelly it also can happen because dublications for Unique index are allowed when dupsAllowed = true
    //  This dublications should be resolved before the end of transaction, but during the transaction we should be
    //  able to keep all RecordIds for the same key.
    //
    //  To be able to distinguish RecordIds related to different keys, we prefix value with it's size.
    ...
    {
        if(_recordFormatter->IsLocalKey())  //  Special hack for the case when we have _id duplication
            MDB_RET(Status(ErrorCodes::DuplicateKey, dupKeyError(BSON("id" << "local_key"))));
    ...
    if (!dupsAllowed)
        MDB_RET(Status(ErrorCodes::DuplicateKey, "Duplicated key"));
...
Status ChunkRocksUniqueIndex::finalizeUnindex(OperationContext* txn, const std::pair<std::string, std::string>& recordKeyValuePair, bool dupsAllowed)
{
    auto ru = _db->GetRocksRecoveryUnit(txn);
    if (!dupsAllowed)
    {
    ...
    {
        warning().stream() << "value {" + recordKeyValuePair.second << "} not found in the index for key {" << recordKeyValuePair.first << "}";
        return Status::OK();
    }
...
Status ChunkRocksUniqueIndex::dupKeyCheck(OperationContext* txn, const BSONObj& key, const RecordId& loc)
{
    std::pair<std::string, std::string> recordKeyValuePair;
    MDB_RIF(GenerateRecord(key, loc, recordKeyValuePair));
...
SortedDataBuilderInterface* ChunkRocksUniqueIndex::getBulkBuilder(OperationContext* txn, bool dupsAllowed)
{
...
//
ChunkRocksStandardIndex::ChunkRocksStandardIndex(mongo::ChunkRocksDBInstance& db, std::unique_ptr<IIndexRecordFormatter> recordFormatter)
    : ChunkRocksIndexBase(db, std::move(recordFormatter)) {}

Status ChunkRocksStandardIndex::onInsert(OperationContext* txn, bool dupsAllowed, std::pair<std::string, std::string>& recordKeyValuePair)
{
    invariant(dupsAllowed); //  For non-unique index dups should allways be allowed
    return Status::OK();
}

Status ChunkRocksStandardIndex::finalizeUnindex(OperationContext* txn, const std::pair<std::string, std::string>& recordKeyValuePair, bool dupsAllowed)
{
    invariant(dupsAllowed); //  For non-unique index dups should always be allowed
    _db->GetRocksRecoveryUnit(txn)->writeBatch()->Delete(GetColumnFamily(), recordKeyValuePair.first);
    return Status::OK();
}

SortedDataBuilderInterface* ChunkRocksStandardIndex::getBulkBuilder(OperationContext* txn, bool dupsAllowed)
{
