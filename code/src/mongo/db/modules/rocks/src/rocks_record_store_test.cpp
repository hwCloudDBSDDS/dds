...
namespace mongo {
    ...
    TEST(RocksRecordStoreTest, Isolation3) {
        std::unique_ptr<HarnessHelper> harnessHelper(newHarnessHelper());
        std::unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());
        ...
        {
            BSONObj shardKey = BSON("Uid" << 1 << "name"
                                          << "gtf");
            ...
            t1.get()->setShardkey(shardKey);
            t2.get()->setShardkey(shardKey);

            ASSERT_OK(rs->updateRecord(t1.get(), loc1, "b", 2, false, NULL));
            ASSERT_OK(rs->updateRecord(t1.get(), loc1, "B", 2, false, NULL));

            // this should throw
            ASSERT_THROWS(rs->updateRecord(t2.get(), loc2, "c", 2, false, NULL),
                          WriteConflictException);
    ...
    StatusWith<RecordId> insertBSON(ServiceContext::UniqueOperationContext& opCtx,
                                    std::unique_ptr<RecordStore>& rs, const Timestamp& opTime) {
        ...
        if (!status.isOK()) return StatusWith<RecordId>(status);
        StatusWith<RecordId> res =
            rs->insertRecord(opCtx.get(), obj.objdata(), obj.objsize(), false);
        if (res.isOK()) wuow.commit();
        return res;
    }
    ...
    TEST(RocksRecordStoreTest, OplogHack) {
...

        {
            ...
            ASSERT_EQ(rs->oplogStartHack(opCtx.get(), RecordId(2, 3)),
                      RecordId(2, 2));  // > highest
        }
    ...
    RecordId _oplogOrderInsertOplog(OperationContext* txn, std::unique_ptr<RecordStore>& rs,
                                    int inc) {
    ...
    TEST(RocksRecordStoreTest, OplogOrder) {
        ...
        // Rollback the last two oplog entries, then insert entries with older optimes and ensure
        // that
        ...
        {
            // Now we insert 2 docs with timestamps earlier than before, but commit the 2nd one
            // first.
            // We make sure we can't find the 2nd until the first is commited.
            ServiceContext::UniqueOperationContext earlyReader(
                harnessHelper->newOperationContext());
