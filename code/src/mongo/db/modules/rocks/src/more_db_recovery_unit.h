...
{
public:
    MoreDbRocksRecoveryUnit(RocksSnapshotManager* snapshotManager, ChunkRocksDBInstance* dbInstance)
...
private:
    ...
    std::map<ChunkRocksDBInstance*, 
             std::unique_ptr<rocksdb::WriteBatchWithIndex> > _writeBatchs;

    std::map<ChunkRocksDBInstance*, CounterMap> _moreDBDeltaCounters;

    std::map<ChunkRocksDBInstance*, RocksTransaction*> _moreTransaction;
};
