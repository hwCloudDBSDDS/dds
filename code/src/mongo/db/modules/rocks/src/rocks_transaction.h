...
namespace mongo {
    ...
    public:
        ...
        void increaseCountOfActiveWriteBatchesOutsideOfSplit() {
        ...
        void subtractCountOfActiveWriteBatchesOutsideOfSplit() {
            _countOfActiveWriteBatchesOutsideOfSplit.fetch_sub(1);
        }

        uint64_t getCountOfActiveWriteBatchesDuringSplit() {
            return _countOfActiveWriteBatchesDuringSplit;
        }

        uint64_t getCountOfActiveWriteBatchesOutsideOfSplit() {
            return _countOfActiveWriteBatchesOutsideOfSplit;
        }

        bool bockingReadIOofRight() const {
            return _blockReadIOofRight.load(std::memory_order_relaxed);
        }

        void setBlockReadIOofRight(bool block) {
            _blockReadIOofRight.store(block, std::memory_order_relaxed);
        }
    ...
    private:
        ...
        std::atomic<bool> _blockReadIOofRight{false};
        std::atomic<bool> _isSplitOnGoing{false};
        std::atomic<uint64_t> _countOfActiveWriteBatchesDuringSplit{0};
        std::atomic<uint64_t> _countOfActiveWriteBatchesOutsideOfSplit{0};
